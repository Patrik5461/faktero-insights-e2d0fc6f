#!/bin/bash
#
# Príprava zdrojov pre Xcode Cloud — samostatná appka Kniha jázd.
#
...
# (viď ios-jazdy/App/CapApp-SPM/Package.swift) a `node_modules` v gite nie je — takže
...
#
# Výskyty sú dva — ladiaca a ostrá konfigurácia jedinej cieľovej aplikácie.
# Faktero ich má štyri, lebo má navyše rozšírenie pre prúžok na uzamknutej
# obrazovke; táto appka ho zatiaľ nemá.
if [ -n "${CI_BUILD_NUMBER:-}" ]; then
  echo "▸ Číslo buildu: ${CI_BUILD_NUMBER}"
  perl -pi -e "s/CURRENT_PROJECT_VERSION = [^;]+;/CURRENT_PROJECT_VERSION = ${CI_BUILD_NUMBER};/g" \
    ios-jazdy/App/App.xcodeproj/project.pbxproj
  grep -c "CURRENT_PROJECT_VERSION = ${CI_BUILD_NUMBER};" ios-jazdy/App/App.xcodeproj/project.pbxproj \
    | xargs -I{} echo "▸ prepísané výskyty: {} (očakávajú sa 2)"
else
...
# `build:jazdy` = klientský build z vite.config.jazdy.ts do dist-jazdy a
# premenovanie index.jazdy.html na index.html. Prekopírovanie do natívneho
# projektu je zvlášť: Capacitor CLI číta vždy `capacitor.config.ts` a vlastný
# `--config` nemá, takže appku volí premenná prostredia. Bez nej by `cap sync`
# nasypal do tohto projektu balíček Faktera.
echo "▸ npm run build:jazdy"
npm run build:jazdy
echo "▸ cap sync (Kniha jázd)"
...
for SUBOR in \
  ...
  "ios-jazdy/App/App/public/index.html" \
  "ios-jazdy/App/App/capacitor.config.json"
do
  if [ ! -e "$SUBOR" ]; then
...
readonly XCPROJEKT="ios-jazdy/App/App.xcodeproj"
...
if [ ! -f "$RESOLVED" ] && command -v swift >/dev/null 2>&1; then
  ...
  ( cd ios-jazdy/App/CapApp-SPM && swift package resolve )
  if [ -f "ios-jazdy/App/CapApp-SPM/Package.resolved" ]; then
    cp "ios-jazdy/App/CapApp-SPM/Package.resolved" "$RESOLVED"
