-- Zálohová faktúra nie je daňový doklad, ale brala číslo z tej istej rady ako
-- vystavené faktúry — v rade potom vznikali diery a z čísla sa nedalo poznať,
-- o aký doklad ide. Dostáva vlastnú radu s predponou ZF; hľadanie voľného
-- čísla ostáva rovnaké, len sa pozerá na doklady svojho druhu.
...
BEGIN
  ...
  -- Kam až hľadať. Počítajú sa len doklady tej istej rady.
  SELECT COUNT(*) + 1
    INTO _strop
   ...
   WHERE company_id = _company_id
     ...
     AND ((_zalohova AND type = 'proforma') OR (NOT _zalohova AND type IS DISTINCT FROM 'proforma'));
