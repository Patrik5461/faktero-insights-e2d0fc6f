ALTER TABLE public.invoices ADD COLUMN IF NOT EXISTS sequence_number integer;

CREATE INDEX IF NOT EXISTS invoices_company_seq_idx
  ON public.invoices (company_id, issue_date, sequence_number);
...
BEGIN
  ...
  -- Determine padding from the longest {N...} token present.
...
REVOKE ALL ON FUNCTION public.faktero_next_invoice_number(uuid, date) FROM PUBLIC;
GRANT EXECUTE ON FUNCTION public.faktero_next_invoice_number(uuid, date) TO authenticated, service_role;