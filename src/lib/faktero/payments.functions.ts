...
export const getMyPaymentProvider = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row } = await supabase
      ...
      .select("id, provider, enabled, sandbox_mode, goid, client_id, connected_at, last_test_at, last_test_ok, last_test_error, updated_at")
      .eq("company_id", data.companyId)
      .eq("provider", "gopay")
      .maybeSingle();
    // also return company toggle
    const { data: co } = await supabase
      .from("companies").select("online_payments_enabled").eq("id", data.companyId).maybeSingle();
...
export const savePaymentProvider = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: isAdmin } = await supabase.rpc("is_company_admin", { _company_id: data.companyId, _user_id: userId });
    ...
    const { data: existing } = await supabaseAdmin
      ...
      .eq("company_id", data.companyId).eq("provider", "gopay").maybeSingle();
...
export const testPaymentProvider = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      await supabaseAdmin.from("company_payment_providers")
        .update({ last_test_at: new Date().toISOString(), last_test_ok: true, last_test_error: null })
        .eq("company_id", data.companyId).eq("provider", "gopay");
      return { ok: true };
    } catch (e: any) {
      const msg = String(e?.message ?? e).slice(0, 500);
      await supabaseAdmin.from("company_payment_providers")
        .update({ last_test_at: new Date().toISOString(), last_test_ok: false, last_test_error: msg })
        .eq("company_id", data.companyId).eq("provider", "gopay");
      throw new Error(msg);
    }
  });
...
export const disconnectPaymentProvider = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    await supabaseAdmin.from("company_payment_providers")
      .delete().eq("company_id", data.companyId).eq("provider", "gopay");
    await supabaseAdmin.from("companies").update({ online_payments_enabled: false }).eq("id", data.companyId);
    await supabaseAdmin.from("platform_audit_logs").insert({
      admin_user_id: userId, action: "payment_provider_disconnect",
      entity_type: "company", entity_id: data.companyId, metadata: { provider: "gopay" } as any,
    });
    return { ok: true };
  });
...
export const setOnlinePaymentsEnabled = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    if (data.enabled) {
      const { data: row } = await supabaseAdmin.from("company_payment_providers")
        .select("enabled").eq("company_id", data.companyId).eq("provider", "gopay").maybeSingle();
      if (!row?.enabled) throw new Error("Najprv pripojte GoPay účet.");
    }
    await supabaseAdmin.from("companies").update({ online_payments_enabled: data.enabled }).eq("id", data.companyId);
    return { ok: true };
  });
...
export const rotateWebhookSecret = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { randomBytes } = await import("crypto");
    const { data: row } = await supabaseAdmin.from("company_payment_providers")
      .select("id").eq("company_id", data.companyId).eq("provider", "gopay").maybeSingle();
...
export const getPaymentDiagnostics = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase, userId } = context;
    const { data: isMember } = await supabase.rpc("is_company_member", { _company_id: data.companyId, _user_id: userId });
    ...
    if (isAdmin) {
      const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
      const { data: adm } = await supabaseAdmin.from("company_payment_providers")
        .select("webhook_secret").eq("company_id", data.companyId).eq("provider", "gopay").maybeSingle();
...
export const syncInvoicePayment = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: link } = await supabaseAdmin.from("invoice_payment_links")
      .select("*").eq("invoice_id", data.invoiceId).eq("company_id", data.companyId)
      .order("created_at", { ascending: false }).limit(1).maybeSingle();
    if (!link) throw new Error("Pre túto faktúru neexistuje žiadny platobný odkaz.");
    if (!link.provider_payment_id) {
      return { state: link.status, message: "Platba ešte nebola spustená (zákazník neotvoril odkaz)." };
    }

    const { data: prov } = await supabaseAdmin.from("company_payment_providers")
      .select("*").eq("company_id", data.companyId).eq("provider", "gopay").maybeSingle();
    ...
    if (isPaid) {
      const { data: inv } = await supabaseAdmin.from("invoices")
        .select("id, status").eq("id", link.invoice_id).maybeSingle();
      ...
      const { data: existingPay } = await supabaseAdmin.from("payments")
        .select("id").eq("invoice_id", link.invoice_id).eq("note", note).maybeSingle();
      if (!existingPay) {
        await supabaseAdmin.from("payments").insert({
          company_id: data.companyId, invoice_id: link.invoice_id,
    ...
    await supabaseAdmin.from("billing_events").insert({
      ...
      payload: { id: String(payment.id), state, invoice_id: link.invoice_id, source: "manual_sync" } as any,
    });
...
export const createInvoicePaymentLink = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: inv, error } = await supabase.from("invoices")
      ...
      .eq("id", data.invoiceId).eq("company_id", data.companyId).maybeSingle();
    if (error || !inv) throw new Error("Faktúra nenájdená.");
    if (inv.status === "cancelled") throw new Error("Stornovanú faktúru nie je možné platiť online.");
    if (inv.status === "paid") throw new Error("Faktúra je už uhradená.");

    const { data: prov } = await supabase.from("company_payment_providers")
      .select("enabled, sandbox_mode").eq("company_id", data.companyId).eq("provider", "gopay").maybeSingle();
    if (!prov?.enabled) throw new Error("Online platby nie sú pre túto firmu pripojené.");

    const { data: co } = await supabase.from("companies")
      .select("online_payments_enabled").eq("id", data.companyId).maybeSingle();
    if (!co?.online_payments_enabled) throw new Error("Online platby sú vypnuté v nastaveniach firmy.");

    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    // Reuse a pending link if it exists and is fresh
    const { data: existing } = await supabaseAdmin.from("invoice_payment_links")
      .select("id, token, status, created_at")
      .eq("invoice_id", data.invoiceId).order("created_at", { ascending: false }).limit(1).maybeSingle();
...
export const getPaymentLinkPublic = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: link } = await supabaseAdmin.from("invoice_payment_links")
      .select("id, invoice_id, company_id, status, amount_cents, currency, sandbox_mode, expires_at, gw_url")
      .eq("token", data.token).maybeSingle();
    if (!link) throw new Error("Platobný odkaz neexistuje.");
    const { data: inv } = await supabaseAdmin.from("invoices")
      .select("invoice_number, due_date, status, variable_symbol").eq("id", link.invoice_id).maybeSingle();
    const { data: co } = await supabaseAdmin.from("companies")
      .select("name").eq("id", link.company_id).maybeSingle();
    return {
      link: {
        status: link.status,
...
export const startPaymentPublic = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data }) => {
    ...
    if (link.expires_at && new Date(link.expires_at) < new Date()) throw new Error("Platnosť odkazu vypršala.");
    if (link.gw_url && link.status === "pending") return { gwUrl: link.gw_url };

    const { data: prov } = await supabaseAdmin.from("company_payment_providers")
      .select("*").eq("company_id", link.company_id).eq("provider", "gopay").maybeSingle();
    if (!prov?.enabled || !prov.encrypted_client_secret) throw new Error("Príjemca platby nemá pripojený GoPay účet.");

    const { data: inv } = await supabaseAdmin.from("invoices")
      .select("invoice_number").eq("id", link.invoice_id).maybeSingle();
