...
function spaydString(opts: { iban: string; amount: number; currency: string; vs?: string | null; msg?: string | null }) {
  const parts = [
    "SPD*1.0",
...
export async function generateInvoicePdfBytes(input: InvoicePdfInput): Promise<Uint8Array> {
  ...
  const numberLabel = input.numberLabel ?? `č. ${invoice.invoice_number ?? invoice.quote_number ?? ""}`;
  ...
  const muted = rgb(0.49, 0.54, 0.60);
  const hairline = rgb(0.88, 0.90, 0.92);
  const surface = rgb(0.972, 0.980, 0.976);
  ...
  page.drawText(san(docLabel), { x: width - margin - titleW, y: y - 8, size: titleSize, font: bold, color: ink });
  const numW = font.widthOfTextAtSize(san(numberLabel), 12);
  page.drawText(san(numberLabel), { x: width - margin - numW, y: y - 30, size: 12, font, color: sub });
  ...
  const partyH = drawPartyCard(page, font, bold, margin, y, colW, "DODÁVATEĽ", {
    name: company.name,
    lines: addressLines(company.street, company.zip, company.city, company.country),
    ico: company.ico, dic: company.dic, ic_dph: company.ic_dph,
    email: company.email, phone: company.phone,
  }, { ink, sub, muted, hairline, primary, surface });
  const partyH2 = drawPartyCard(page, font, bold, margin + colW + gap, y, colW, "ODBERATEĽ", {
    name: invoice.customer_name,
    lines: addressLines(invoice.customer_street, invoice.customer_zip, invoice.customer_city, invoice.customer_country),
    ico: invoice.customer_ico, dic: invoice.customer_dic, ic_dph: invoice.customer_ic_dph,
  ...
  meta.forEach(([k, v], i) => {
    ...
    if (i > 0) {
      page.drawLine({ start: { x: margin + i * metaW, y: y - 8 }, end: { x: margin + i * metaW, y: y - metaBoxH + 8 }, color: hairline, thickness: 0.5 });
    }
  });
  y -= metaBoxH + 22;
  ...
  const NAME_SIZE = 10, DESC_SIZE = 8.5, ROW_LINE_H = 12;
  const ROW_PAD_Y = 8;
  ...
  const drawTableHeader = (p: PDFPage, top: number): number => {
    const headerH = 26;
    p.drawRectangle({ x: margin, y: top - headerH, width: innerW, height: headerH, color: rgb(0.96, 0.97, 0.965) });
    const baseY = top - 17;
  ...
  for (let idx = 0; idx < items.length; idx++) {
    ...
    y -= rowH;
    cur.drawLine({ start: { x: margin, y }, end: { x: width - margin, y }, color: hairline, thickness: 0.5 });
  }
  ...
  if (!input.hidePayment) {
    ...
    cur.drawRectangle({ x: payX, y: payY - payH, width: payW, height: payH, color: white, borderColor: hairline, borderWidth: 0.7 });
    cur.drawRectangle({ x: payX, y: payY - payH, width: 3, height: payH, color: primary });
    cur.drawText("PLATOBNÉ ÚDAJE", { x: payX + 16, y: payY - 20, size: 8, font: bold, color: muted });
    ...
    if (company.iban) {
      try {
        const qrText = spaydString({
          iban: company.iban, amount: Number(invoice.total), currency: invoice.currency,
          vs: invoice.variable_symbol, msg: `Faktura ${invoice.invoice_number}`,
        });
        ...
        cur.drawText("QR PLATBA PREVODOM", { x: qrX, y: qrY + qrSize + 6, size: 7, font: bold, color: muted });
      } catch { /* ignore */ }
    }
    y = payY - payH - 24;
    ...
    if (input.paymentLinkUrl) {
      ...
      cur.drawRectangle({ x: gpX, y: gpY - gpH, width: gpW, height: gpH, color: white, borderColor: hairline, borderWidth: 0.7 });
      cur.drawRectangle({ x: gpX, y: gpY - gpH, width: 3, height: gpH, color: primary });
      cur.drawText("ONLINE PLATBA GOPAY", { x: gpX + 16, y: gpY - 20, size: 8, font: bold, color: muted });
      ...
      cur.drawText(btnLabel, { x: btnX + btnPadX, y: btnY - btnH + 8, size: 11, font: bold, color: white });

      cur.drawText("Naskenujte QR kód alebo otvorte odkaz:", {
        x: gpX + 16, y: btnY - btnH - 16, size: 8.5, font, color: muted,
      });
      const linkStr = ellipsize(String(input.paymentLinkUrl), font, 9, gpW - gpQR - 64);
      cur.drawText(linkStr, {
        x: gpX + 16, y: btnY - btnH - 30, size: 9, font: bold, color: primaryDark,
      });

      try {
        const dataUrl = await QRCode.toDataURL(String(input.paymentLinkUrl), { margin: 0, width: 240 });
        const png = await doc.embedPng(dataUrl);
        const qrX = gpX + gpW - gpQR - 16;
        const qrY = gpY - gpH + (gpH - gpQR) / 2;
        cur.drawImage(png, { x: qrX, y: qrY, width: gpQR, height: gpQR });
        cur.drawText("ONLINE PLATBA GOPAY", { x: qrX - 8, y: qrY + gpQR + 6, size: 7, font: bold, color: muted });
      } catch { /* ignore */ }
  ...
  pages.forEach((p, i) => {
    p.drawLine({ start: { x: margin, y: 52 }, end: { x: width - margin, y: 52 }, color: hairline, thickness: 0.5 });
...
function wrapLines(text: string, f: PDFFont, size: number, maxW: number): string[] {
  ...
  const hardSplit = (w: string): string[] => {
    ...
    let cur = "";
    for (const ch of w) {
      if (widthOf(cur + ch) > maxW && cur) { parts.push(cur); cur = ch; }
      else cur += ch;
    }
  ...
  for (const w of words) {
    ...
    if (widthOf(trial) > maxW) { out.push(line); line = w; }
    else line = trial;
  }
...
function drawAligned(page: PDFPage, font: PDFFont, text: string, x: number, y: number, size: number, color: any, align: "left" | "right") {
  const w = font.widthOfTextAtSize(text, size);
  page.drawText(text, { x: align === "right" ? x - w : x, y, size, font, color });
}

function drawTotalRow(page: PDFPage, font: PDFFont, label: string, value: string, x: number, y: number, w: number, ink: any, muted: any) {
...
function drawPartyCard(
  ...
  label: string, p: { name?: string; lines: string[]; ico?: string; dic?: string; ic_dph?: string; email?: string; phone?: string },
  ...
  if (ids.length) {
    cy -= 6;
    page.drawLine({ start: { x: x + padX, y: cy }, end: { x: x + w - padX, y: cy }, color: c.hairline, thickness: 0.5 });
    cy -= 4;
    ids.forEach(([k, v]) => {
  ...
  page.drawRectangle({ x, y: y - cardH, width: w, height: cardH, borderColor: c.hairline, borderWidth: 0.7 });
  return cardH;
}
