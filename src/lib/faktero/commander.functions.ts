...
export const getCommanderStatus = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: row } = await supabase
      ...
      .select(
        "id, enabled, username, auto_sync_daily, last_sync_at, sync_status, error_message, updated_at",
      )
    ...
    const { data: links } = await supabase
      ...
      .select(
        "id, commander_vehicle_id, commander_vehicle_name, commander_license_plate, faktero_vehicle_id, last_synced_at",
      )
      .eq("company_id", data.companyId)
    ...
    return {
      connection: row
        ? {
            ...row,
            username_masked: maskUser(row.username),
            username: undefined,
            credentials_invalid,
          }
        : null,
...
async function logSync(
  ...
  try {
    ...
    console.error("[commander-log]", e);
  }
}

const DECRYPT_MSG =
  "Prihlasovacie údaje Commander GPS je potrebné znova zadať. Toto sa stáva po zmene bezpečnostného kľúča systému. Zadajte prosím váš Commander username a heslo znova.";
...
export const testCommander = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    try {
      ...
      const msg =
        e instanceof CommanderAuthError
          ? "Neplatné prihlasovacie údaje."
          : e instanceof CommanderRateLimitError
            ? "Commander API limit bol dosiahnutý. Skúste neskôr."
...
export const syncCommanderVehicles = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    await assertAdmin(context, data.companyId);
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { commanderListVehicles, CommanderAuthError, CommanderRateLimitError, mapFuelType } =
      await import("./commander.server");
    const { isDecryptError } = await import("./payment-crypto.server");
    let creds;
    try {
      creds = await loadCreds(data.companyId);
    } catch (e: any) {
      if (isDecryptError(e)) {
    ...
    let vehicles;
    try {
      vehicles = await commanderListVehicles(creds.username, creds.password);
    } catch (e: any) {
      const msg =
        e instanceof CommanderAuthError
          ? "Neplatné prihlasovacie údaje."
    ...
    try {
        ...
        for (const r of rides) {
          ...
          if (rawStart === null || rawStart === undefined || rawStart === "") {
            skip(
              ...
              r,
              mappedLink,
              `Chýba začiatočný dátum. Kľúče v payloade: [${Object.keys(r ?? {}).join(", ")}]`,
            );
            continue;
          }
          if (!start) {
            skip(
              ...
              r,
              mappedLink,
              `Neplatný začiatočný dátum. Raw: ${JSON.stringify(rawStart)} (typ: ${typeof rawStart}). Parser vrátil null.`,
            );
            continue;
          }
          ...
          if (externalId) {
            ...
            if (dupError) {
              skip(
                ...
                r,
                mappedLink,
                `Kontrola duplicity zlyhala: ${dupError.message}`,
              );
              continue;
            }
            if (dup) {
              skip(
                ...
                r,
                mappedLink,
                `external_id ${externalId} už existuje.`,
              );
              continue;
            }
          } else if (!data.force) {
      ...
      const msg =
        e instanceof CommanderAuthError
          ? "Neplatné prihlasovacie údaje."
          : e instanceof CommanderRateLimitError
            ? "Commander API limit bol dosiahnutý. Skúste synchronizáciu neskôr."
      ...
      return {
        ...
        unlinked: skippedBreakdown.vehicle_not_linked,
        errors: skippedBreakdown.validation_error + skippedBreakdown.insert_error,
        fetched,
      };
    }
