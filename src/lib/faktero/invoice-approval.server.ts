...
export async function sendApprovalRequestEmail(params: {
  ...
  const { getEmailTemplate, applyTemplateVars, DEFAULT_TEMPLATES } =
...
export async function sendApprovalResultEmail(params: {
  ...
  const total = `${Number(invoice.total).toFixed(2)} ${invoice.currency}`;
  const text = `Faktúra ${invoice.invoice_number} pre odberateľa ${invoice.customer_name ?? ""} v sume ${total} bola ${status}.
${!approved && note ? `\nDôvod zamietnutia:\n${note}\n` : ""}
...
export async function getInvoiceForApproval(token: string) {
  const { data: inv } = await supabaseAdmin
    .from("invoices")
    .select(
      "id, invoice_number, customer_name, customer_email, subtotal, vat_total, total, currency, issue_date, due_date, status, approval_status, approval_requested_at, approval_responded_at, approval_note, company_id, notes",
    )
  ...
  const [{ data: items }, { data: company }] = await Promise.all([
    ...
    supabaseAdmin
      .from("companies")
      .select("name, street, city, zip, country, ico, dic, ic_dph, email, logo_url")
