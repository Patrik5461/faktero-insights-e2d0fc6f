...
export const convertQuoteToInvoice = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: quote, error } = await supabase
      .from("quotes").select("*").eq("id", data.quoteId).single();
    ...
    const { data: inv, error: insErr } = await supabase.from("invoices").insert({
      ...
      customer_dic: quote.customer_dic, customer_ic_dph: quote.customer_ic_dph,
      customer_street: quote.customer_street, customer_city: quote.customer_city,
      customer_zip: quote.customer_zip, customer_country: quote.customer_country ?? "SK",
      customer_email: quote.customer_email,
      subtotal: quote.subtotal, vat_total: quote.vat_total, total: quote.total,
    ...
    if (items && items.length) {
      const rows = items.map((it: any) => ({
        invoice_id: inv.id, position: it.position,
        name: it.name, description: it.description,
        quantity: it.quantity, unit: it.unit, unit_price: it.unit_price, vat_rate: it.vat_rate,
        subtotal: it.subtotal, vat_amount: it.vat_amount, total: it.total,
      }));
    ...
    try {
      ...
      await triggerEvent({ company_id: quote.company_id, event: "invoice.created", data: invoicePayload(inv) });
    } catch { /* non-fatal */ }
...
export const duplicateQuote = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { supabase } = context;
    const { data: q, error } = await supabase.from("quotes").select("*").eq("id", data.quoteId).single();
    ...
    const { data: copy, error: insErr } = await supabase.from("quotes").insert({
      ...
      currency: q.currency,
      notes: q.notes,
      customer_name: q.customer_name, customer_ico: q.customer_ico,
      customer_dic: q.customer_dic, customer_ic_dph: q.customer_ic_dph,
      customer_street: q.customer_street, customer_city: q.customer_city,
      customer_zip: q.customer_zip, customer_country: q.customer_country,
      customer_email: q.customer_email,
      subtotal: q.subtotal, vat_total: q.vat_total, total: q.total,
    }).select().single();
    if (insErr || !copy) throw new Error(insErr?.message ?? "Duplikácia zlyhala");

    if (items && items.length) {
      const rows = items.map((it: any) => ({
        quote_id: copy.id, position: it.position,
        name: it.name, description: it.description,
        quantity: it.quantity, unit: it.unit, unit_price: it.unit_price, vat_rate: it.vat_rate,
        subtotal: it.subtotal, vat_amount: it.vat_amount, total: it.total,
      }));
