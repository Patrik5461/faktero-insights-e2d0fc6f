...
export const createImportUploadUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const ok = await context.supabase.rpc("is_company_member", { _company_id: data.companyId, _user_id: context.userId });
    ...
    const { data: signed, error } = await supabaseAdmin.storage.from("imports").createSignedUploadUrl(path);
...
export const previewImport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (!ok.data) throw new Error("Nemáte prístup k firme.");
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: blob, error: dlErr } = await supabaseAdmin.storage.from("imports").download(data.path);
    if (dlErr || !blob) throw new Error("Súbor sa nepodarilo načítať.");
    const bytes = new Uint8Array(await blob.arrayBuffer());
    const { extractTables, suggestMapping, buildPreview, detectMapping } = await import("./import-superfaktura.server");
    ...
    const mapping = (data.mapping && Object.keys(data.mapping).length
      ? data.mapping
      : detection.mapping) as Record<string, string>;
...
export const executeImport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: job, error: jobErr } = await supabaseAdmin.from("import_jobs").insert({
...
export const getImportFileUrl = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    const { data: job, error } = await context.supabase.from("import_jobs").select("file_path, company_id").eq("id", data.jobId).single();
    if (error || !job?.file_path) throw new Error("Súbor nenájdený.");
    const { supabaseAdmin } = await import("@/integrations/supabase/client.server");
    const { data: signed, error: sErr } = await supabaseAdmin.storage.from("imports").createSignedUrl(job.file_path, 600);
