...
async function maybeSyncVehicles(companyId: string, username: string, password: string, lastSyncAt: string | null) {
  ...
  const { data: faktVehicles } = await supabaseAdmin
    .from("vehicles").select("id, name, license_plate").eq("company_id", companyId);
...
async function syncCompanyDaily(companyId: string, username: string, password: string, lastSyncAt: string | null) {
  ...
  if (!links?.length) {
    return { imported, duplicates: 0, fetched, candidates, skippedBreakdown, skippedRides, note: "Žiadne prepojené vozidlá." };
  }
  ...
  for (const link of links) {
    const rides = await commanderListRides(username, password, String(link.commander_vehicle_id), from, to);
    ...
    for (const r of rides) {
      ...
      if (!start) { skip("validation_error", r, mappedLink, `Neplatný začiatočný dátum. Raw: ${JSON.stringify(rawStart)} (typ: ${typeof rawStart}). Kľúče: [${Object.keys(r ?? {}).join(", ")}]`); continue; }
      ...
      if (!Number.isFinite(distance) || distance < 0) { skip("validation_error", r, mappedLink, `Neplatná vzdialenosť: ${String(resolvedDistance ?? "")}`); continue; }
      if (end_odo < start_odo) { skip("validation_error", r, mappedLink, `Koncový tachometer (${end_odo}) je menší ako počiatočný (${start_odo}).`); continue; }

      if (externalId) {
        const { data: dup, error: dupError } = await supabaseAdmin.from("trips").select("id")
          .eq("company_id", companyId).eq("external_source", "commander").eq("external_id", externalId).maybeSingle();
      ...
      if (error) { skip("insert_error", r, mappedLink, error.message); continue; }
      imported++;
    }
  }

  const duplicates = skippedBreakdown.duplicate_external_id + skippedBreakdown.duplicate_fallback_match;
  ...
  return { imported, duplicates, fetched, candidates, skippedBreakdown, skippedRides, note: null as string | null };
}
...
export async function runCommanderDailySync(): Promise<DailyResult> {
  ...
  for (const c of conns ?? []) {
    result.processed++;
    let password: string;
    try { password = decryptSecret(c.encrypted_password); }
    catch (e: any) {
      ...
      await supabaseAdmin.from("commander_connections").update({
        sync_status: "error", error_message: msg,
    ...
    try {
      ...
      const msg = r.note ?? `Denná synchronizácia: ${r.imported} importovaných, ${r.duplicates} duplikátov.`;
      await writeLog(c.company_id, "ok", msg, {
        ...
        skipped_rides_count: Object.values(r.skippedBreakdown).reduce((sum, count) => sum + count, 0),
