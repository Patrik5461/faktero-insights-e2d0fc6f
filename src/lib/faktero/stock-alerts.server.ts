...
export async function runLowStockAlerts() {
  ...
  for (const c of companies ?? []) {
    ...
    if (prodIds.length) {
      const { data: prods } = await supabaseAdmin.from("products").select("id, name").in("id", prodIds);
      (prods ?? []).forEach((p: any) => nameMap.set(p.id, p.name));
    }
    let recipient = c.email as string | null;
    if (!recipient && c.created_by) {
      const { data: prof } = await supabaseAdmin.from("profiles").select("email").eq("id", c.created_by).maybeSingle();
      recipient = prof?.email ?? null;
    }
    if (!recipient) { results.push({ company_id: c.id, skipped: true, reason: "no_recipient" }); continue; }

    const rowsHtml = low.map((i: any) => {
      const name = i.product_id ? nameMap.get(i.product_id) ?? i.sku ?? "—" : i.sku ?? "—";
      const cur = totals.get(i.id) ?? 0;
      return `<tr><td style="padding:6px 8px;border-bottom:1px solid #eee">${escapeHtml(name)}</td><td style="padding:6px 8px;border-bottom:1px solid #eee">${escapeHtml(i.sku ?? "")}</td><td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right;color:#b45309"><strong>${cur.toFixed(2)}</strong> ${escapeHtml(i.unit ?? "")}</td><td style="padding:6px 8px;border-bottom:1px solid #eee;text-align:right">${Number(i.min_stock).toFixed(2)}</td></tr>`;
    }).join("");
    const rowsTxt = low.map((i: any) => {
      const name = i.product_id ? nameMap.get(i.product_id) ?? i.sku ?? "—" : i.sku ?? "—";
      return `- ${name} (${i.sku ?? "-"}): ${(totals.get(i.id) ?? 0).toFixed(2)} / min ${Number(i.min_stock).toFixed(2)}`;
    }).join("\n");

    const html = `<div style="font-family:system-ui,sans-serif;max-width:640px;margin:0 auto"><h2 style="color:#b45309">⚠️ Skladové položky pod minimom</h2><p>Firma <strong>${escapeHtml(c.name)}</strong> má ${low.length} položiek pod minimálnym stavom.</p><table style="width:100%;border-collapse:collapse;font-size:14px"><thead><tr style="background:#f8fafc"><th style="padding:8px;text-align:left">Produkt</th><th style="padding:8px;text-align:left">SKU</th><th style="padding:8px;text-align:right">Stav</th><th style="padding:8px;text-align:right">Minimum</th></tr></thead><tbody>${rowsHtml}</tbody></table><p style="margin-top:16px"><a href="${process.env.APP_PUBLIC_URL || "https://www.faktero.sk"}/sklad/minimum" style="background:#0f172a;color:#fff;padding:10px 16px;border-radius:6px;text-decoration:none">Zobraziť v Faktere</a></p></div>`;
    const text = `Skladové položky pod minimom (${c.name}):\n\n${rowsTxt}\n\nOtvorte: ${process.env.APP_PUBLIC_URL || "https://www.faktero.sk"}/sklad/minimum`;
    try {
      await sendMail({ to: recipient, subject: `⚠️ ${low.length} položiek pod minimom — ${c.name}`, html, text });
