...
async function assertMember(ctx: any, companyId: string) {
  const ok = await ctx.supabase.rpc("is_company_member", { _company_id: companyId, _user_id: ctx.userId });
...
export const previewVendorImport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    if (rows.length === 0) throw new Error("V súbore sa nenašli žiadne faktúry v očakávanom formáte.");
...
export const executeVendorImport = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const canonicalKeys: string[] = [
      ...
      "customer_name","customer_ico","customer_dic","customer_ic_dph","customer_email",
      "customer_phone","customer_street","customer_city","customer_zip","customer_country",
