...
const TEMPLATE_TYPES = ["invoice_send", "reminder_1", "reminder_2", "reminder_3", "approval_request"] as const;
...
export const sendTestEmailTemplateFn = createServerFn({ method: "POST" })
  ...
  .handler(async ({ data, context }) => {
    ...
    const { data: co } = await context.supabase
      .from("companies").select("id, name, iban, email_sender_name, email_reply_to")
    ...
    const subject = "[TEST] " + applyTemplateVars(data.subject, { invoice: sampleInvoice, company: co });
