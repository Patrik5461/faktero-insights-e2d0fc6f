...
export function MovementForm({ type, title, onDone }: { type: MovementType; title: string; onDone?: () => void }) {
  ...
  useEffect(() => {
    ...
    (async () => {
      setLoading(true);
      let { data: wh } = await supabase.from("warehouses").select("id, name").eq("company_id", cid).eq("active", true).order("name");
      if (!wh || wh.length === 0) {
        const { data: created, error } = await supabase.from("warehouses").insert({ company_id: cid, name: "Hlavný sklad", active: true }).select().single();
        if (created) { wh = [created]; toast.success("Vytvorený sklad 'Hlavný sklad'."); }
        else if (error) toast.error(error.message);
      }
      const [{ data: si }, { data: prods }] = await Promise.all([
        supabase.from("stock_items").select("id, sku, product_id, sale_price, purchase_price").eq("company_id", cid).order("sku"),
  ...
  async function submit(e: React.FormEvent) {
    ...
    try {
      ...
      if (!(result as any).ok) {
        ...
        if (m.includes("FAKTERO_STOCK:negative_stock")) return toast.error("Pohyb by spôsobil záporný stav skladu." + (SHOW_STOCK_DEBUG ? ` (${m})` : ""));
        if (m.includes("FAKTERO_STOCK:zero_quantity")) return toast.error("Množstvo nemôže byť 0." + (SHOW_STOCK_DEBUG ? ` (${m})` : ""));
        if (m.includes("FAKTERO_STOCK:cross_company")) return toast.error("Sklad a položka musia patriť tej istej firme." + (SHOW_STOCK_DEBUG ? ` (${m})` : ""));
        if (m.includes("FAKTERO_STOCK:warehouse_inactive")) return toast.error("Sklad je neaktívny — pohyb nie je možný." + (SHOW_STOCK_DEBUG ? ` (${m})` : ""));
        if (m.includes("FAKTERO_STOCK:movement_immutable")) return toast.error("Skladové pohyby nie je možné upravovať ani mazať. Vytvorte opravný pohyb." + (SHOW_STOCK_DEBUG ? ` (${m})` : ""));
        if (m.includes("FAKTERO_STOCK:invoice_item_locked")) return toast.error("Položky ovplyvňujúce sklad nie je možné meniť po odoslaní faktúry." + (SHOW_STOCK_DEBUG ? ` (${m})` : ""));
        if (m.includes("FAKTERO_PLAN_BLOCK")) return toast.error("Vaše predplatné nie je aktívne. Pre pokračovanie si aktivujte plán." + (SHOW_STOCK_DEBUG ? ` (${m})` : ""));
        return toast.error(SHOW_STOCK_DEBUG ? `Sklad chyba: ${m}` : m);
      }
      toast.success("Pohyb uložený."); onDone?.();
    } catch (error: any) {
      ...
      <PageBody>
        {!loading && items.length === 0 ? (
          <div className="mx-auto max-w-xl rounded-xl border border-border bg-card p-8 text-center">
            ...
            <Link to="/sklad/produkty" className="mt-3 inline-flex items-center gap-2 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:opacity-90">
              Pridať tovar
            </Link>
          </div>
        ) : (
        <form onSubmit={submit} className="mx-auto grid max-w-xl gap-3 rounded-xl border border-border bg-card p-6">
          <label className="block">
            <span className="text-sm font-medium">Sklad *</span>
            <select required value={warehouse} onChange={(e) => setWarehouse(e.target.value)} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
              {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
            </select>
          </label>
          <label className="block">
            <span className="text-sm font-medium">Tovar / skladová karta *</span>
            <select required value={stockItem} onChange={(e) => {
          ...
          <div className="grid grid-cols-2 gap-3">
            <label className="block">
              ...
              <input type="number" step="0.001" min="0.001" required value={quantity} onChange={(e) => setQuantity(e.target.value)} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" />
            </label>
            <label className="block">
          ...
          {SHOW_STOCK_DEBUG && debug && (
            <pre className="max-h-56 overflow-auto rounded-md border border-amber-300/50 bg-amber-50 p-3 text-[11px] text-amber-950 dark:bg-amber-950/20 dark:text-amber-100">{JSON.stringify(debug, null, 2)}</pre>
          )}
        </form>
        )}
      </PageBody>
