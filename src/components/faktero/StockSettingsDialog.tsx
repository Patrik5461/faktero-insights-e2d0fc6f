      ...
      >
        ...
        <div className="max-h-[70vh] overflow-y-auto p-5">
          {tab === "kategorie" ? (
            ...
            <WarehousesTab onChanged={onChanged} />
          )}
        </div>
      </div>
    </div>
    ...
    <div className="grid gap-4 md:grid-cols-[1fr_18rem]">
        ...
        <table className="w-full text-sm">
          <thead className="bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground">
            <tr>
              ...
              <th className="p-2 text-right">Produkty</th>
              <th className="p-2"></th>
            </tr>
          </thead>
              ...
              <tr key={r.id}>
                ...
                <td className="p-2 text-right">
                  <div className="inline-flex gap-1">
                    ...
                    <button
                      onClick={() => remove(r.id, r.product_count)}
      ...
      <div className="rounded-lg border border-border p-3">
        ...
        <div className="mt-1 flex flex-wrap gap-1.5">
          <button
            onClick={() => setColor("")}
            className={`h-6 w-6 rounded-full border ${color === "" ? "ring-2 ring-primary" : "border-border"}`}
            title="Bez farby"
          />
          {PALETTE.map((c) => (
            <button
              key={c}
              onClick={() => setColor(c)}
              className={`h-6 w-6 rounded-full border border-border ${color === c ? "ring-2 ring-primary" : ""}`}
              style={{ background: c }}
            />
          ))}
        </div>
          ...
          <tbody className="divide-y divide-border">
            ...
            {rows.map((w) => (
              <tr key={w.id}>
                ...
                <td className="p-2 text-right">
                  <button onClick={() => setEditing(w)} className="rounded p-1 hover:bg-muted">
                    <Pencil className="h-3.5 w-3.5" />
                  </button>
                </td>
              </tr>
