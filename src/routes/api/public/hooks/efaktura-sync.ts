/**
 * Cron: nočné sťahovanie prijatých eFaktúr a doplnenie stavov odoslaných.
 * Volaný cez pg_cron s hlavičkou `x-faktero-cron-token: <FAKTERO_CRON_TOKEN>`.
 */
import { createFileRoute } from "@tanstack/react-router";

export const Route = createFileRoute("/api/public/hooks/efaktura-sync")({
  server: {
    handlers: {
      POST: async ({ request }) => {
        const token =
        ...
        try {
          const { spustiNocnuEfakturu } = await import("@/lib/faktero/efaktura/nocna-uloha.server");
          const r = await spustiNocnuEfakturu();
