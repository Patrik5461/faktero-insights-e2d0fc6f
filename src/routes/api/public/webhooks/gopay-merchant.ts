  ...
  try {
    ...
    if (!paymentId && request.method === "POST") {
      ...
      try {
        if (ct.includes("json")) {
          ...
          paymentId = b?.id ? String(b.id) : null;
        } else {
          const t = await request.text();
          paymentId = new URLSearchParams(t).get("id");
        }
      } catch {
        /* ignore */
      }
    ...
    if (!link || link.company_id !== cid) {
      await supabaseAdmin.from("billing_events").insert({
        company_id: cid,
        event_type: "gopay_merchant_unknown_payment",
    ...
    await supabaseAdmin.from("billing_events").insert({
      company_id: cid,
      event_type: `gopay_merchant_${state.toLowerCase() || "unknown"}`,
    ...
    try {
      ...
      await supabaseAdmin.from("billing_events").insert({
        company_id: null,
        event_type: "gopay_merchant_webhook_error",
