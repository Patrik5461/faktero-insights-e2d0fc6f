...
export const Route = createFileRoute("/api/v1/quotes")({
  server: {
    handlers: {
      ...
      POST: async ({ request }) => {
        const { handleApi, ok, err } = await import("@/lib/faktero/api-auth.server");
        const { nextQuoteNumber, computeQuoteTotals } =
          await import("@/lib/faktero/quote-numbering.server");
        return handleApi(request, async (ctx) => {
          const parsed = QuoteInput.safeParse(ctx.requestBody);
          if (!parsed.success)
            return err("validation_error", "Neplatné dáta ponuky.", 400, parsed.error.flatten());
          const d = parsed.data;
          ...
          const valid_until =
            d.valid_until ?? new Date(Date.now() + 30 * 86400000).toISOString().slice(0, 10);
