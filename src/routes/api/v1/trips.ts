    ...
    handlers: {
      ...
      POST: async ({ request }) => {
        ...
        return handleApi(request, async (ctx) => {
          ...
          if (!parsed.success)
            return err("validation_error", "Neplatné dáta jazdy.", 400, parsed.error.flatten());
          const d = parsed.data;
          if (d.end_odometer < d.start_odometer)
            return err("validation_error", "end_odometer musí byť ≥ start_odometer.", 400);
          ...
          const fuel_consumption = v.consumption_l_100km
            ? (distance * Number(v.consumption_l_100km)) / 100
            : null;
          const { data, error } = await ctx.supabase
            .from("trips")
            .insert({
