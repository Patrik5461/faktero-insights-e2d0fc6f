    ...
    handlers: {
      ...
      GET: async ({ request }) => {
        ...
        return handleApi(request, async (ctx) => {
          ...
          const { data, error } = await ctx.supabase.from("customers").select("*").eq("company_id", ctx.company_id).order("created_at", { ascending: false }).limit(limit);
          if (error) return { status: 500, body: { error: { code: "db_error", message: error.message } } };
          return ok({ data });
        });
      },
      POST: async ({ request }) => {
        const { handleApi, ok, err } = await import("@/lib/faktero/api-auth.server");
        const { triggerEvent, customerPayload } = await import("@/lib/faktero/webhook-trigger.server");
        return handleApi(request, async (ctx) => {
          const parsed = CustomerInput.safeParse(ctx.requestBody);
          if (!parsed.success) return err("validation_error", "Neplatné dáta odberateľa.", 400, parsed.error.flatten());
          if (parsed.data.external_id) {
            const { data: existing } = await ctx.supabase.from("customers").select("*")
              .eq("company_id", ctx.company_id).eq("external_id", parsed.data.external_id).maybeSingle();
            if (existing) return ok(existing, 200);
          }
          const { data, error } = await ctx.supabase.from("customers")
            .insert({ ...parsed.data, company_id: ctx.company_id }).select().single();
          if (error) return err("db_error", error.message, 500);
          await triggerEvent({ company_id: ctx.company_id, event: "customer.created", data: customerPayload(data) });
          return ok(data, 201);
        });
      },
    },
