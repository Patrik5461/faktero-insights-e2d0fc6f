    ...
    handlers: {
      POST: async ({ request, params }) => {
        ...
        return handleApi(request, async (ctx) => {
          ...
          const { data: q } = await ctx.supabase.from("quotes").select("*")
            .eq("company_id", ctx.company_id).eq("id", id).maybeSingle();
          if (!q) return err("not_found", "Ponuka nenájdená.", 404);
          if (q.status === "converted" && q.converted_invoice_id) {
            const { data: inv } = await ctx.supabase.from("invoices").select("*").eq("id", q.converted_invoice_id).maybeSingle();
          ...
          const { data: inv, error: insErr } = await ctx.supabase.from("invoices").insert({
            ...
            customer_zip: q.customer_zip, customer_country: q.customer_country ?? "SK",
            customer_email: q.customer_email,
            subtotal: q.subtotal, vat_total: q.vat_total, total: q.total,
            notes: q.notes ?? null, status: "issued",
          }).select().single();
          if (insErr || !inv) return err("db_error", insErr?.message ?? "Insert zlyhal", 500);
          if (items?.length) {
            const rows = items.map((it: any) => ({
              invoice_id: inv.id, position: it.position, name: it.name, description: it.description,
          ...
          await ctx.supabase.from("quotes").update({
            status: "converted", converted_invoice_id: inv.id, converted_at: new Date().toISOString(),
          }).eq("id", q.id);
          try {
            const { triggerEvent, invoicePayload } = await import("@/lib/faktero/webhook-trigger.server");
            await triggerEvent({ company_id: ctx.company_id, event: "invoice.created", data: invoicePayload(inv) });
          } catch {}
          return ok(inv, 201);
        });
      },
