    ...
    handlers: {
      GET: async ({ request }) => {
        ...
        return handleApi(request, async (ctx) => {
          const { data, error } = await ctx.supabase.from("recurring_invoices").select("*")
            .eq("company_id", ctx.company_id).order("created_at", { ascending: false });
      ...
      POST: async ({ request }) => {
        ...
        return handleApi(request, async (ctx) => {
          ...
          const d = parsed.data;
          const { data: cust } = await ctx.supabase.from("customers").select("*")
          ...
          const { data: created, error: insErr } = await ctx.supabase.from("recurring_invoices").insert({
            company_id: ctx.company_id, name: d.name, customer_id: cust.id,
            customer_name: cust.name, customer_ico: cust.ico, customer_dic: cust.dic, customer_ic_dph: cust.ic_dph,
            customer_street: cust.street, customer_city: cust.city, customer_zip: cust.zip,
            customer_country: cust.country, customer_email: cust.email,
            frequency: d.frequency, next_run: d.next_run, currency: d.currency,
            due_days: d.due_days, payment_method: d.payment_method, notes: d.notes, active: d.active,
            items: d.items as any, ...t,
          }).select().single();
