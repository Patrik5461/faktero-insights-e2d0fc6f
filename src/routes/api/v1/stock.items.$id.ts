    ...
    handlers: {
      ...
      GET: async ({ request, params }) => {
        ...
        return handleApi(request, async (ctx) => {
          const { data: item, error } = await ctx.supabase.from("stock_items").select("*").eq("company_id", ctx.company_id).eq("id", params.id).maybeSingle();
          if (error) return err("db_error", error.message, 500);
          if (!item) return err("not_found", "Skladová karta neexistuje.", 404);
          const { data: levels } = await ctx.supabase.from("stock_levels").select("warehouse_id, quantity, reserved_quantity").eq("stock_item_id", params.id);
