    ...
    handlers: {
      POST: async ({ request, params }) => {
        ...
        return handleApi(request, async (ctx) => {
          ...
          const { data: inv } = await ctx.supabase.from("invoices")
            .select("id, company_id, customer_email, status")
            .eq("id", params.id).eq("company_id", ctx.company_id).maybeSingle();
          if (!inv) return err("not_found", "Faktúra nenájdená.", 404);
          if (inv.status === "cancelled") return err("invalid_state", "Stornovanú faktúru nemožno odoslať.", 409);
          ...
          try {
            ...
            const r = await sendInvoiceEmail({
              company_id: ctx.company_id, invoice_id: inv.id,
              recipient_email: to, subject: parsed.data.subject, message: parsed.data.message,
            });
