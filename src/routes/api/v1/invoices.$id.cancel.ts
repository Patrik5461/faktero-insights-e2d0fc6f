    ...
    handlers: {
      POST: async ({ request, params }) => {
        ...
        return handleApi(request, async (ctx) => {
          ...
          if (existing.status === "cancelled") return err("invalid_state", "Faktúra je už stornovaná.", 409);
          ...
          await triggerEvent({ company_id: ctx.company_id, event: "invoice.cancelled", data: invoicePayload(data) });
          return ok(data);
        });
      },
    },
