...
function AdminSubscriptionsPage() {
  ...
  const [diag, setDiag] = useState<{ events: any[]; eventsTotal: number; page: number; pageSize: number; failedPayments: any[] } | null>(null);
  ...
  async function onExtend(companyId: string) {
    ...
    try { await extendFn({ data: { companyId, days } }); toast.success("Trial predĺžený"); await reload(); }
    catch (e: any) { toast.error(e?.message ?? "Chyba"); }
  }
  async function onCancel(companyId: string) {
    if (!confirm("Zrušiť predplatné?")) return;
    try { await cancelFn({ data: { companyId } }); toast.success("Zrušené"); await reload(); }
    catch (e: any) { toast.error(e?.message ?? "Chyba"); }
  }
  async function onReactivate(companyId: string) {
    try { await reactivateFn({ data: { companyId } }); toast.success("Reaktivované"); await reload(); }
    catch (e: any) { toast.error(e?.message ?? "Chyba"); }
  }
  async function onMarkActive(companyId: string) {
    const days = Number(window.prompt("Označiť aktívne na (dní):", "30"));
    if (!days || days < 1) return;
    try { await markActiveFn({ data: { companyId, days } }); toast.success("Označené ako aktívne"); await reload(); }
    catch (e: any) { toast.error(e?.message ?? "Chyba"); }
  }
  async function onSuspend(companyId: string, suspend: boolean) {
    ...
    try { await suspendFn({ data: { companyId, suspend } }); toast.success(suspend ? "Pozastavené" : "Obnovené"); await reload(); }
    catch (e: any) { toast.error(e?.message ?? "Chyba"); }
  }
      ...
      <AdminPageBody>
              ...
              <table className="w-full text-sm">
                ...
                <tbody>
                  {loading && rows.length === 0 ? (
                    <tr><td colSpan={8} className="px-3 py-6 text-center text-muted-foreground">Načítavam…</td></tr>
                  ) : rows.length === 0 ? (
                    <tr><td colSpan={8} className="px-3 py-6 text-center text-muted-foreground">Žiadne predplatné.</td></tr>
                  ) : rows.map((r) => (
                    <tr key={r.id} className="border-t border-border hover:bg-muted/30">
                      <td className="px-3 py-2 font-medium">{r.companies?.name ?? "—"}</td>
                      <td className="px-3 py-2 uppercase">{r.plan ?? "free"}</td>
                      <td className="px-3 py-2">{r.status ?? "—"}</td>
                      <td className="px-3 py-2 text-muted-foreground">{fmtDate(r.trial_ends_at)}</td>
                      <td className="px-3 py-2 text-muted-foreground">{fmtDate(r.next_billing_at)}</td>
                      <td className="px-3 py-2 text-right tabular-nums">{fmtPrice(r.monthly_price_cents)}</td>
                      ...
                      <td className="px-3 py-2">
                        <div className="flex flex-wrap gap-1 text-xs">
                          <button onClick={() => onSetPlan(r.company_id)} className="rounded border border-border px-2 py-0.5 hover:bg-muted">Plán</button>
                          <button onClick={() => onExtend(r.company_id)} className="rounded border border-border px-2 py-0.5 hover:bg-muted">+Trial</button>
                          <button onClick={() => onMarkActive(r.company_id)} className="rounded border border-border px-2 py-0.5 hover:bg-muted">Aktívne</button>
                          <button onClick={() => onSuspend(r.company_id, true)} className="rounded border border-border px-2 py-0.5 hover:bg-muted">Pozastaviť</button>
                          <button onClick={() => onSuspend(r.company_id, false)} className="rounded border border-border px-2 py-0.5 hover:bg-muted">Obnoviť</button>
                          {r.status === "cancelled" ? (
                            <button onClick={() => onReactivate(r.company_id)} className="rounded border border-border px-2 py-0.5 hover:bg-muted">Reaktivovať</button>
                          ) : (
                            <button onClick={() => onCancel(r.company_id)} className="rounded border border-border px-2 py-0.5 hover:bg-muted">Zrušiť</button>
                          )}
                        </div>
                      </td>
                    </tr>
        ...
        {total > pageSize && (
          <div className="mt-4 flex items-center justify-between text-sm">
            <span className="text-muted-foreground">Strana {page} z {pages} · {total}</span>
          ...
          {config && (
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              <dl className="grid grid-cols-1 gap-1 text-xs sm:grid-cols-2">
                <div><dt className="text-muted-foreground">APP_PUBLIC_URL</dt><dd className="font-mono">{config.app_public_url ?? "—"}</dd></div>
                <div><dt className="text-muted-foreground">GOPAY_ENV</dt><dd className="font-mono">{config.gopay_env ?? "—"}</dd></div>
                <div><dt className="text-muted-foreground">Webhook secret</dt><dd className="font-mono">{config.webhook_secret_set ? config.webhook_secret_preview : "—"}</dd></div>
                <div className="sm:col-span-2"><dt className="text-muted-foreground">Notify URL pre GoPay</dt><dd className="font-mono break-all">{config.webhook_notify_url ?? "—"}</dd></div>
              </dl>
              <p className="mt-3 text-xs text-muted-foreground">
                Po zmene <span className="font-mono">GOPAY_WEBHOOK_SECRET</span> aktualizujte notification URL v GoPay konzole.
              </p>
              {config.warnings?.length > 0 && (
                <ul className="mt-3 space-y-1 text-xs text-destructive">
                  {config.warnings.map((w: string, i: number) => <li key={i}>⚠ {w}</li>)}
                </ul>
              )}
            </div>
          )}
                ...
                <ul className="space-y-1 text-xs">
                  {diag.events.map((e) => (
                    <li key={e.id} className="flex flex-wrap items-baseline gap-2">
                      <span className="text-muted-foreground tabular-nums">{fmtDate(e.created_at)}</span>
                      <span className="font-mono">{e.event_type}</span>
                      <span className="text-muted-foreground">{e.payload?.id ? `· id ${e.payload.id}` : ""}</span>
                      <span className="text-muted-foreground">{e.payload?.state ? `· ${e.payload.state}` : ""}</span>
                      {e.payload?.error && <span className="text-destructive">· {String(e.payload.error)}</span>}
                    </li>
                  ))}
                </ul>
                {diag.eventsTotal > diagPageSize && (
                  <div className="mt-3 flex items-center justify-between text-xs">
                    <span className="text-muted-foreground">Strana {diag.page} z {Math.max(1, Math.ceil(diag.eventsTotal / diagPageSize))} · {diag.eventsTotal}</span>
                    <div className="flex gap-2">
                      <button disabled={diagPage <= 1} onClick={() => setDiagPage((p) => Math.max(1, p - 1))} className="rounded-md border border-border px-2 py-1 disabled:opacity-50">← Späť</button>
                      <button disabled={diagPage >= Math.ceil(diag.eventsTotal / diagPageSize)} onClick={() => setDiagPage((p) => p + 1)} className="rounded-md border border-border px-2 py-1 disabled:opacity-50">Ďalej →</button>
                    </div>
                  </div>
                )}
              </>
                  ...
                  {diag.failedPayments.map((p) => (
                    <tr key={p.id} className="border-t border-border">
                      ...
                      <td className="py-1 text-right">
                        <button onClick={() => onSyncPayment(p.provider_payment_id)} className="rounded border border-border px-2 py-0.5 hover:bg-muted">
                          Sync
                        </button>
                      </td>
                    </tr>
