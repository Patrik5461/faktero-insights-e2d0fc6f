...
function PayPage() {
  ...
  async function onPay() {
    ...
    try {
      ...
      setError(e?.message ?? "Platbu sa nepodarilo spustiť.");
    } finally {
      setBusy(false);
    }
  }
  ...
  if (error && !data)
    return (
      <PageWrap>
        <Center>
          <h1 className="text-xl font-semibold mb-2">Odkaz nie je dostupný</h1>
        ...
        <div className="rounded-2xl border border-border bg-card p-8 shadow-sm">
          ...
          <div className="my-6 text-center">
            <div className="text-3xl font-bold tracking-tight">
              {fmt(data.link.amountCents, data.link.currency)}
            </div>
            {data.invoice.dueDate && (
              <p className="mt-1 text-xs text-muted-foreground">
                Splatnosť do {data.invoice.dueDate}
              </p>
            )}
          </div>
            ...
            <>
              <label className="block text-sm">
                <span className="text-muted-foreground">
                  Váš e-mail (voliteľné, pre potvrdenie)
                </span>
                <input
                  type="email"
                  value={email}
          ...
          <div className="mt-6 flex items-start gap-2 text-xs text-muted-foreground">
            ...
            <span>
              Platba prebieha priamo cez GoPay. Peniaze idú na účet príjemcu — nie cez Faktero.
            </span>
          </div>
        </div>
      </div>
...
function PageWrap({ children }: { children: React.ReactNode }) {
  return (
    <div className="min-h-screen bg-gradient-to-b from-emerald-50/30 to-background">{children}</div>
  );
}
