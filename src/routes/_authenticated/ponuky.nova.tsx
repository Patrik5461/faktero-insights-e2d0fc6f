...
function NewQuote() {
  ...
  useEffect(() => {
    ...
    if (!cid) return;
    supabase.from("customers").select("id, name, ico, dic, ic_dph, street, city, zip, country, email").eq("company_id", cid).then(({ data }) => setCustomers(data ?? []));
  }, []);
  ...
  async function genNumber(cid: string) {
    ...
    const { count } = await supabase.from("quotes").select("id", { count: "exact", head: true })
      .eq("company_id", cid).gte("issue_date", `${yr}-01-01`);
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { data: q, error } = await supabase.from("quotes").insert({
      company_id: cid, customer_id: cust.id, status: "draft", quote_number,
      issue_date: form.issue_date, valid_until: form.valid_until, currency: form.currency,
    ...
    const rows = items.map((it, i) => {
      ...
      return {
        quote_id: q.id, position: i, name: it.name, description: it.description,
        quantity: it.quantity, unit: it.unit, unit_price: it.unit_price, vat_rate: it.vat_rate,
        subtotal: +s.toFixed(2), vat_amount: +v.toFixed(2), total: +(s + v).toFixed(2),
      };
    });
        ...
        <form onSubmit={submit} className="space-y-6">
          <div className="grid gap-4 rounded-xl border border-border bg-card p-5 sm:grid-cols-3">
            <label className="block sm:col-span-3">
              ...
              <div className="mt-1 flex gap-2">
                <select required value={form.customer_id} onChange={(e) => setForm({ ...form, customer_id: e.target.value })} className="w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
            ...
            <In label="Dátum vystavenia" type="date" value={form.issue_date} onChange={(v) => setForm({ ...form, issue_date: v })} />
            <In label="Platnosť do" type="date" value={form.valid_until} onChange={(v) => setForm({ ...form, valid_until: v })} />
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            <div className="mb-3 flex items-center justify-between">
              ...
              <button type="button" onClick={() => setItems([...items, { ...EMPTY }])} className="inline-flex items-center gap-1 rounded-md border border-border px-3 py-1.5 text-sm hover:bg-secondary">
