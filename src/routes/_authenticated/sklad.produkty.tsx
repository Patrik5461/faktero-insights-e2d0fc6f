...
type NewProduct = {
  ...
  track_stock: boolean; min_stock: number;
  initial_quantity: number; warehouse_id: string;
};
...
function StockItemsPage() {
  ...
  async function load() {
    ...
    const perWh: Record<string, Array<{ warehouse_id: string; quantity: number; reserved: number }>> = {};
    (lvl ?? []).forEach((l: any) => {
  ...
  useEffect(() => {
    if (!editing?.id) { setRecentMovements([]); return; }
    (async () => {
  ...
  async function save(s: SI) {
    ...
    const op = s.id ? supabase.from("stock_items").update(payload).eq("id", s.id) : supabase.from("stock_items").insert(payload);
  ...
  async function createProduct(n: NewProduct) {
    ...
    setSaving(true);
    try {
      const result = await createProductWithDebug({ data: { company_id: cid, ...n, warehouse_id: n.warehouse_id || null } });
      const rawDebug = (result as any).debug;
      console.info("[sklad-debug:add-goods:client-result]", result);
      setDebug((prev: any) => ({
        ...(prev ?? {}),
        ...(rawDebug ?? {}),
        last_stock_error: (result as any).ok ? null : ((result as any).error ?? rawDebug?.exact_error?.message ?? "Neznáma chyba"),
        last_created_product_id: (result as any).product?.id ?? rawDebug?.product_insert_result?.data?.id ?? prev?.last_created_product_id ?? null,
        last_created_stock_item_id: (result as any).stockItem?.id ?? rawDebug?.stock_item_insert_result?.data?.id ?? prev?.last_created_stock_item_id ?? null,
        last_movement_id: (result as any).movement?.id ?? rawDebug?.stock_movement_insert_result?.data?.id ?? prev?.last_movement_id ?? null,
      ...
      setDebug((prev: any) => ({ ...(prev ?? {}), last_stock_error: e?.message ?? String(e), last_raw_debug: e }));
  ...
  const filtered = rows.filter((r) => {
    if (search && !((r.sku ?? "").toLowerCase().includes(search.toLowerCase()) || (r.barcode ?? "").toLowerCase().includes(search.toLowerCase()))) return false;
  ...
  function buildExportRows(): { headers: string[]; rows: ExportRow[] } {
    const headers = [
      ...
      "Aktuálny stav", "Rezervované množstvo", "Minimálny stav",
      "Nákupná cena", "Predajná cena", "DPH %", "Hodnota skladu", "Sledovanie skladu",
    ];
    ...
    for (const s of filtered) {
      ...
      const sources = warehouseFilter ? perWh.filter((l) => l.warehouse_id === warehouseFilter) : perWh;
      if (sources.length === 0) {
        out.push({
          "SKU": s.sku ?? "", "Názov produktu": prod?.name ?? "",
          "Čiarový kód": s.barcode ?? "", "Jednotka": s.unit ?? "ks", "Sklad": "—",
          "Aktuálny stav": 0, "Rezervované množstvo": 0,
          "Minimálny stav": Number(s.min_stock), "Nákupná cena": Number(s.purchase_price),
          "Predajná cena": Number(s.sale_price), "DPH %": Number(s.vat_rate),
          "Hodnota skladu": 0, "Sledovanie skladu": s.track_stock ? "áno" : "nie",
        });
        continue;
      }
      for (const l of sources) {
        out.push({
          "SKU": s.sku ?? "", "Názov produktu": prod?.name ?? "",
          "Čiarový kód": s.barcode ?? "", "Jednotka": s.unit ?? "ks", "Sklad": whName(l.warehouse_id),
          "Aktuálny stav": l.quantity, "Rezervované množstvo": l.reserved,
          "Minimálny stav": Number(s.min_stock), "Nákupná cena": Number(s.purchase_price),
          "Predajná cena": Number(s.sale_price), "DPH %": Number(s.vat_rate),
  ...
  return (
    <>
      <PageHeader title="Skladové položky" description="Skladové karty napojené na produkty." action={
          ...
          <div className="relative">
            ...
            {exportOpen && (
              <div className="absolute right-0 z-20 mt-1 w-44 rounded-md border border-border bg-popover p-1 shadow-md">
                <button onClick={() => exportStock("csv")} className="block w-full rounded px-3 py-2 text-left text-sm hover:bg-muted">CSV (.csv)</button>
                <button onClick={() => exportStock("xlsx")} className="block w-full rounded px-3 py-2 text-left text-sm hover:bg-muted">Excel (.xlsx)</button>
              </div>
            )}
          </div>
          <Link to="/sklad/import" className="inline-flex items-center gap-2 rounded-md border border-border bg-card px-3 py-2 text-sm font-medium hover:bg-secondary">
        ...
        {SHOW_STOCK_DEBUG && (
          <div className="mb-4 rounded-lg border border-amber-300/50 bg-amber-50 p-3 text-xs text-amber-950 dark:bg-amber-950/20 dark:text-amber-100">
            ...
            <div className="grid gap-1 sm:grid-cols-2 lg:grid-cols-4">
              <DebugLine label="active company_id" value={debug?.company_id ?? getActiveCompanyId() ?? "—"} />
              <DebugLine label="warehouses count" value={debug?.warehouses_count ?? warehouses.length} />
              <DebugLine label="stock_items count" value={debug?.stock_items_count ?? rows.length} />
              <DebugLine label="stock_movements count" value={debug?.stock_movements_count ?? "—"} />
              <DebugLine label="last stock error" value={debug?.last_stock_error ?? debug?.exact_error?.message ?? "—"} />
              <DebugLine label="last created product id" value={debug?.last_created_product_id ?? debug?.product_insert_result?.data?.id ?? "—"} />
              <DebugLine label="last created stock item id" value={debug?.last_created_stock_item_id ?? debug?.stock_item_insert_result?.data?.id ?? "—"} />
              <DebugLine label="last movement id" value={debug?.last_movement_id ?? debug?.stock_movement_insert_result?.data?.id ?? "—"} />
            </div>
            {debug?.last_raw_debug && <pre className="mt-2 max-h-48 overflow-auto rounded bg-background/70 p-2 text-[11px]">{JSON.stringify(debug.last_raw_debug, null, 2)}</pre>}
          </div>
        )}
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <div className="flex flex-wrap items-center gap-3">
            <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Hľadať SKU, čiarový kód…" className="w-64 rounded-md border border-input bg-background px-3 py-2 text-sm" />
      ...
      {editing && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={() => setEditing(null)}>
          <div className="w-full max-w-2xl rounded-xl border border-border bg-card p-6" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold">{editing.id ? "Upraviť skladovú kartu" : "Nová skladová karta"}</h2>
            <form onSubmit={(e) => { e.preventDefault(); save(editing); }} className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="block sm:col-span-2">
                <span className="text-sm font-medium">Produkt</span>
                <select value={editing.product_id ?? ""} onChange={(e) => setEditing({ ...editing, product_id: e.target.value || null })} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
                  <option value="">— bez napojenia —</option>
                  {products.map((p) => <option key={p.id} value={p.id}>{p.name}</option>)}
                </select>
              </label>
              <In label="SKU" value={editing.sku ?? ""} onChange={(v) => setEditing({ ...editing, sku: v })} />
              <In label="Čiarový kód" value={editing.barcode ?? ""} onChange={(v) => setEditing({ ...editing, barcode: v })} />
              <In label="Nákupná cena" type="number" value={String(editing.purchase_price)} onChange={(v) => setEditing({ ...editing, purchase_price: Number(v) })} />
              <In label="Predajná cena" type="number" value={String(editing.sale_price)} onChange={(v) => setEditing({ ...editing, sale_price: Number(v) })} />
              <In label="DPH %" type="number" value={String(editing.vat_rate)} onChange={(v) => setEditing({ ...editing, vat_rate: Number(v) })} />
              <In label="MJ" value={editing.unit} onChange={(v) => setEditing({ ...editing, unit: v })} />
              <In label="Minimálny stav" type="number" value={String(editing.min_stock)} onChange={(v) => setEditing({ ...editing, min_stock: Number(v) })} />
            ...
            {editing.id && (
              <div className="mt-5 rounded-lg border border-border bg-muted/20 p-4">
                <div className="mb-2 flex items-center justify-between">
                  <div className="inline-flex items-center gap-2 text-sm font-medium"><History className="h-4 w-4" /> Posledné pohyby</div>
                  <Link to="/sklad/pohyby" search={{ stock_item_id: editing.id }} className="text-xs text-primary hover:underline">Všetky pohyby →</Link>
                </div>
                  ...
                  <ul className="space-y-1 text-xs">
                    {recentMovements.map((m) => (
                      <li key={m.id} className="flex items-center justify-between">
                        <span className="text-muted-foreground">{new Date(m.created_at).toLocaleString("sk-SK")}</span>
                        <span className="font-medium">{m.type}</span>
                        <span className={Number(m.quantity) >= 0 ? "text-emerald-600 tabular-nums" : "text-destructive tabular-nums"}>{Number(m.quantity) > 0 ? "+" : ""}{m.quantity}</span>
                      </li>
                    ))}
                  </ul>
                )}
      ...
      {creating && (
        <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4 overflow-y-auto" onClick={() => !saving && setCreating(null)}>
          <div className="w-full max-w-2xl rounded-xl border border-border bg-card p-6 my-8" onClick={(e) => e.stopPropagation()}>
            <h2 className="text-lg font-semibold">Pridať tovar</h2>
            <p className="mt-1 text-xs text-muted-foreground">Vytvorí produkt a skladovú kartu. Ak zadáte počiatočný stav, vytvorí sa aj príjem.</p>
            <form onSubmit={(e) => { e.preventDefault(); createProduct(creating); }} className="mt-4 grid gap-3 sm:grid-cols-2">
              <label className="block sm:col-span-2">
                <span className="text-sm font-medium">Názov *</span>
                <input required autoFocus value={creating.name} onChange={(e) => setCreating({ ...creating, name: e.target.value })} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" />
              </label>
              <In label="SKU" value={creating.sku} onChange={(v) => setCreating({ ...creating, sku: v })} />
              <In label="Čiarový kód" value={creating.barcode} onChange={(v) => setCreating({ ...creating, barcode: v })} />
              <In label="Jednotka" value={creating.unit} onChange={(v) => setCreating({ ...creating, unit: v })} />
              <In label="DPH %" type="number" value={String(creating.vat_rate)} onChange={(v) => setCreating({ ...creating, vat_rate: Number(v) || 0 })} />
              <In label="Predajná cena" type="number" value={String(creating.sale_price)} onChange={(v) => setCreating({ ...creating, sale_price: Number(v) || 0 })} />
              <In label="Nákupná cena" type="number" value={String(creating.purchase_price)} onChange={(v) => setCreating({ ...creating, purchase_price: Number(v) || 0 })} />
              <In label="Minimálny stav" type="number" value={String(creating.min_stock)} onChange={(v) => setCreating({ ...creating, min_stock: Number(v) || 0 })} />
              <label className="sm:col-span-2 inline-flex items-center gap-2 text-sm">
                <input type="checkbox" checked={creating.track_stock} onChange={(e) => setCreating({ ...creating, track_stock: e.target.checked })} />
                Sledovať sklad
              </label>
              <div className="sm:col-span-2 mt-2 rounded-lg border border-border bg-muted/20 p-3">
                <div className="mb-2 text-xs font-medium uppercase tracking-wide text-muted-foreground">Počiatočný stav (voliteľné)</div>
                <div className="grid gap-3 sm:grid-cols-2">
                  <label className="block">
                    <span className="text-sm font-medium">Sklad</span>
                    <select value={creating.warehouse_id} onChange={(e) => setCreating({ ...creating, warehouse_id: e.target.value })} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm">
                      {warehouses.map((w) => <option key={w.id} value={w.id}>{w.name}</option>)}
                    </select>
                  </label>
                  <In label="Počiatočné množstvo" type="number" value={String(creating.initial_quantity)} onChange={(v) => setCreating({ ...creating, initial_quantity: Number(v) || 0 })} />
                </div>
              </div>
              <div className="sm:col-span-2 flex justify-end gap-2 pt-2">
                <button type="button" disabled={saving} onClick={() => setCreating(null)} className="rounded-md border border-border bg-card px-4 py-2 text-sm hover:bg-secondary disabled:opacity-60">Zrušiť</button>
...
function DebugLine({ label, value }: { label: string; value: React.ReactNode }) {
  return <div><span className="font-medium">{label}:</span> <span className="break-all">{value}</span></div>;
}