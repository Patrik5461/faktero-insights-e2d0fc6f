  ...
  if (loading || !data) {
    return (
      <>
        ...
        <PageBody><div className="rounded-md border p-6 text-sm text-muted-foreground">Načítavam…</div></PageBody>
      </>
    );
  }

  const { transfer, items, stockItems, warehouses, companies, products } = data;
  const whMap: Record<string, any> = Object.fromEntries((warehouses ?? []).map((w: any) => [w.id, w]));
  const coMap: Record<string, any> = Object.fromEntries((companies ?? []).map((c: any) => [c.id, c]));
  const siMap: Record<string, any> = Object.fromEntries((stockItems ?? []).map((s: any) => [s.id, s]));
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <Link to="/sklad/presuny" className="inline-flex items-center gap-2 rounded-md border px-3 py-2 text-sm">
        ...
        <div className="mx-auto max-w-4xl space-y-5">
          <div className="rounded-md border p-4">
            <div className="flex items-center justify-between">
              <span className={`inline-block rounded px-2 py-0.5 text-xs ${STATUS_STYLE[transfer.status] ?? ""}`}>
            ...
            <div className="mt-4 flex items-center gap-3 text-sm">
              <div>
                ...
                <div className="text-xs text-muted-foreground">{coMap[transfer.company_id]?.name ?? ""}</div>
              </div>
              <ArrowRightLeft className="h-5 w-5 text-muted-foreground" />
              <div>
                <div className="text-xs text-muted-foreground">Do</div>
                <div className="font-medium">{transfer.warehouse_to_id ? whMap[transfer.warehouse_to_id]?.name ?? "—" : "—"}</div>
                <div className="text-xs text-muted-foreground">
                  {transfer.target_company_id ? coMap[transfer.target_company_id]?.name ?? "Iná firma" : coMap[transfer.company_id]?.name ?? ""}
                </div>
              </div>
            </div>
            {transfer.note && <div className="mt-3 text-sm text-muted-foreground">{transfer.note}</div>}
          </div>
            ...
            <table className="w-full text-sm">
                ...
                {items.map((it: any) => {
                  ...
                  return (
                    <tr key={it.id} className="border-t">
                      ...
                      <td className="px-3 py-2 text-right">{(Number(it.quantity) * Number(it.unit_price)).toFixed(2)} €</td>
                      {transfer.target_company_id && (
                        <td className="px-3 py-2 text-muted-foreground">
                          {tgt ? tgt.sku ?? tgt.id.slice(0, 8) : transfer.status === "draft" ? "Bude priradená pri dokončení" : "—"}
                        </td>
                      )}
                    </tr>
                  );
              ...
              <tfoot>
                <tr className="border-t bg-muted/30 font-medium">
                  <td className="px-3 py-2" colSpan={4}>Spolu</td>
