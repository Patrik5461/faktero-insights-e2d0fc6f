...
import {
  getCommanderStatus, saveCommanderConnection, disconnectCommander, testCommander,
  syncCommanderVehicles, linkCommanderVehicle, syncCommanderRides,
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">Údaje sú uložené šifrovane a používané iba na serveri.</div>
                </div>
              </div>
              ...
              {conn && (
                <div className={`mt-3 rounded-md border p-3 text-xs ${conn.auto_sync_daily ? "border-emerald-500/30 bg-emerald-500/10 text-emerald-700 dark:text-emerald-300" : "border-border bg-muted text-muted-foreground"}`}>
              ...
              <div className="mt-5 flex flex-wrap gap-2">
                <button disabled={!!busy} onClick={onTest} className="inline-flex items-center gap-2 rounded-md border border-border bg-background px-3 py-2 text-sm hover:bg-muted disabled:opacity-50">
                  <TestTube2 className="h-4 w-4" /> Otestovať pripojenie
                </button>
                <button disabled={!!busy} onClick={onSave} className="inline-flex items-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">
              ...
              <div className="mb-3 flex items-center justify-between">
                <div>
                  ...
                  <div className="text-xs text-muted-foreground">
                    {state.links.length === 0 ? "Žiadne vozidlá ešte neboli synchronizované." : `${state.links.length} vozidiel z Commandera.`}
                  </div>
                </div>
                <button disabled={!conn || !!busy} onClick={onSyncVehicles} className="inline-flex items-center gap-2 rounded-md border border-border bg-background px-3 py-2 text-sm hover:bg-muted disabled:opacity-50">
                ...
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    <thead className="text-left text-xs uppercase text-muted-foreground">
                      <tr><th className="py-2 pr-3">Commander vozidlo</th><th className="py-2 pr-3">EČV</th><th className="py-2 pr-3">Faktero vozidlo</th></tr>
                    </thead>
                    <tbody>
                      {state.links.map((l: any) => (
                        <tr key={l.id} className="border-t border-border">
                          <td className="py-2 pr-3">{l.commander_vehicle_name ?? l.commander_vehicle_id}</td>
                          <td className="py-2 pr-3 text-muted-foreground">{l.commander_license_plate ?? "—"}</td>
            ...
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <div className="flex flex-wrap items-end gap-3">
                <div>
                  ...
                  <select value={preset} onChange={(e) => setPreset(e.target.value as any)} className="rounded-md border border-input bg-background px-3 py-2 text-sm">
                ...
                <button disabled={!conn || !!busy} onClick={onSyncRides} className="inline-flex items-center gap-2 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">
          ...
          <div className="space-y-4">
            <div className="rounded-xl border border-border bg-card p-5">
              ...
              <dl className="space-y-2 text-sm">
                <div className="flex justify-between gap-3"><dt className="text-muted-foreground">Pripojenie</dt><dd>{conn ? (conn.enabled ? "Aktívne" : "Vypnuté") : "Nepripojené"}</dd></div>
                <div className="flex justify-between gap-3"><dt className="text-muted-foreground">Používateľ</dt><dd className="font-mono">{conn?.username_masked ?? "—"}</dd></div>
                      ...
                      {(l.sync_type === "rides" || l.sync_type === "daily") && l.raw_response && (
                        <div className="mt-2 space-y-2 text-[11px] text-muted-foreground">
                          <div className="grid grid-cols-2 gap-1">
                            <div>Načítané: <span className="text-foreground font-medium">{l.raw_response.fetched_rides_count ?? 0}</span></div>
                            <div>Kandidáti: <span className="text-foreground font-medium">{l.raw_response.candidate_rides_count ?? l.raw_response.mapped_rides_count ?? 0}</span></div>
                            <div>Imported: <span className="text-foreground font-medium">{l.raw_response.inserted_trips_count ?? 0}</span></div>
                            <div>Duplicate: <span className="text-foreground font-medium">{l.raw_response.skipped_duplicates ?? ((l.raw_response.duplicate_external_id ?? 0) + (l.raw_response.duplicate_fallback_match ?? 0))}</span></div>
                            <div>Vehicle not linked: <span className="text-foreground font-medium">{l.raw_response.skipped_unlinked_vehicle ?? 0}</span></div>
                            <div>Validation error: <span className="text-foreground font-medium">{l.raw_response.validation_errors ?? 0}</span></div>
                            <div>Insert error: <span className="text-foreground font-medium">{l.raw_response.insert_errors ?? 0}</span></div>
                            <div>Missing mapping: <span className="text-foreground font-medium">{l.raw_response.missing_vehicle_mapping ?? 0}</span></div>
                          </div>
                          {Array.isArray(l.raw_response.skipped_rides) && l.raw_response.skipped_rides.length > 0 && (
                            <div className="max-h-56 overflow-auto rounded border border-border bg-muted/40 p-2">
                              <div className="mb-1 font-medium text-foreground">Dôvody preskočenia ({l.raw_response.skipped_rides.length})</div>
                              <div className="space-y-1 font-mono text-[10px]">
                                {l.raw_response.skipped_rides.map((ride: any, idx: number) => (
                                  <div key={`${ride.external_id ?? "ride"}-${idx}`} className="border-t border-border pt-1 first:border-t-0 first:pt-0">
                                    <span className="text-foreground">{ride.reason}</span>
                                    {ride.external_id ? <span> · ride {ride.external_id}</span> : null}
                                    {ride.commander_vehicle_id ? <span> · vehicle {ride.commander_vehicle_id}</span> : null}
                                    {ride.detail ? <div className="whitespace-pre-wrap">{ride.detail}</div> : null}
                                  </div>
                                ))}
                              </div>
                            </div>
                          )}
                          {Array.isArray(l.raw_response.sample_rides) && l.raw_response.sample_rides.length > 0 && (
                            <div className="max-h-56 overflow-auto rounded border border-border bg-muted/40 p-2">
                              <div className="mb-1 font-medium text-foreground">Vzorka jázd ({l.raw_response.sample_rides.length}) — surové vs. parsované dátumy</div>
                              <div className="space-y-1 font-mono text-[10px]">
                                {l.raw_response.sample_rides.map((s: any, idx: number) => (
                                  <div key={`${s.ride_id ?? "s"}-${idx}`} className="border-t border-border pt-1 first:border-t-0 first:pt-0">
                                    <div>ride {s.ride_id ?? "—"} · vehicle {s.vehicle_id ?? "—"} · typ {s.datetimeStart_type}</div>
                                    {Array.isArray(s.keys) && <div>keys: [{s.keys.join(", ")}]</div>}
                                    <div>start raw: {JSON.stringify(s.datetimeStart_raw)} → {s.datetimeStart_parsed ?? "null"}</div>
                                    <div>end&nbsp;&nbsp; raw: {JSON.stringify(s.datetimeEnd_raw)} → {s.datetimeEnd_parsed ?? "null"}</div>
                                    <div>distance: {s.distance ?? "null"} · start_loc: {s.start_location ?? "null"} · end_loc: {s.end_location ?? "null"}</div>
                                  </div>
                                ))}
                              </div>
                            </div>
                          )}
                          {Array.isArray(l.raw_response.raw_ride_sample_full) && l.raw_response.raw_ride_sample_full.length > 0 && (
                            <div className="max-h-72 overflow-auto rounded border border-border bg-muted/40 p-2">
                              <div className="mb-1 font-medium text-foreground">Surová Commander odpoveď ({l.raw_response.raw_ride_sample_full.length} jázd)</div>
                              <pre className="whitespace-pre-wrap break-all font-mono text-[10px]">{JSON.stringify(l.raw_response.raw_ride_sample_full, null, 2)}</pre>
                            </div>
                          )}
                        </div>
                      )}
