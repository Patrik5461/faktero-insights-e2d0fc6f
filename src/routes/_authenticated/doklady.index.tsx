...
import {
  listExpensesFn, deleteExpenseFn, exportExpensesZipFn, getExpenseFileUrlFn,
...
const STATUS_LABEL: Record<string, string> = {
  new: "Nový", processed: "Spracovaný", exported: "Exportovaný",
};
...
function DokladyPage() {
  ...
  async function refresh() {
    ...
    try {
      ...
      setRows(data ?? []);
    } catch (e: any) { toast.error(e?.message ?? "Chyba pri načítaní"); }
    finally { setLoading(false); }
  }
  useEffect(() => { refresh(); /* eslint-disable-next-line */ }, [status, month]);
  ...
  async function handleExport(markExported: boolean) {
    ...
    try {
      ...
      const res = await exportFn({ data: { company_id: cid, ids, month: ids ? null : month, mark_exported: markExported } });
      ...
      a.href = url; a.download = res.filename; a.click(); URL.revokeObjectURL(url);
      toast.success(`Exportovaných ${res.count} dokladov`);
      if (markExported) refresh();
    } catch (e: any) { toast.error(e?.message ?? "Export zlyhal"); }
  ...
  async function openFile(path: string) {
    ...
    } catch (e: any) { toast.error(e?.message ?? "Nedá sa otvoriť súbor"); }
  }

  async function del(id: string) {
    if (!confirm("Naozaj zmazať doklad?")) return;
    try { await deleteFn({ data: { id } }); refresh(); }
    catch (e: any) { toast.error(e?.message ?? "Zmazanie zlyhalo"); }
  }
      ...
      <PageHeader
        ...
        action={
          <div className="flex gap-2">
            <Link to="/doklady/novy" className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground hover:opacity-90">
      ...
      <PageBody>
        <div className="mb-4 flex flex-wrap items-end gap-3">
          ...
          <div className="ml-auto flex gap-2">
            <button onClick={() => handleExport(false)} disabled={exporting || !rows.length}
            ...
            <button onClick={() => handleExport(true)} disabled={exporting || !rows.length}
          ...
          {loading ? (
            ...
            <div className="p-12 text-center text-sm text-muted-foreground">
              ...
              <div className="mt-4 flex justify-center gap-2">
                <Link to="/doklady/novy" className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground">
            ...
            <table className="w-full text-sm">
              <thead className="bg-secondary/40 text-xs text-muted-foreground">
                <tr>
                  <th className="w-8 px-3 py-2"><input type="checkbox" checked={selected.size === rows.length} onChange={toggleAll} /></th>
                ...
                {rows.map((r) => (
                  <tr key={r.id} className="border-t border-border">
                    ...
                    <td className="px-3 py-2">
                      <button onClick={() => navigate({ to: "/doklady/novy", search: { id: r.id } as any })} className="text-left hover:underline">
                        {r.supplier_name ?? <span className="text-muted-foreground italic">bez názvu</span>}
                      </button>
                      {r.supplier_ico ? <div className="text-xs text-muted-foreground">IČO {r.supplier_ico}</div> : null}
                    </td>
                    ...
                    <td className="px-3 py-2">
                      <span className={`inline-flex rounded-md px-2 py-0.5 text-xs font-medium ${STATUS_STYLE[r.status]}`}>
                    ...
                    <td className="px-3 py-2 text-right">
                      {r.file_path && (
                        <button onClick={() => openFile(r.file_path)} title="Otvoriť súbor" className="rounded-md p-1.5 hover:bg-secondary">
                          <FileText className="h-4 w-4" />
                        </button>
                      )}
                      <button onClick={() => del(r.id)} title="Zmazať" className="rounded-md p-1.5 hover:bg-destructive/10">
...
function SummaryCard({ label, value, highlight }: { label: string; value: number; highlight?: boolean }) {
  return (
    <div className={`rounded-xl border border-border p-4 ${highlight ? "bg-primary/5" : "bg-card"}`}>
