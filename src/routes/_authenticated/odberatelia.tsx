...
type Customer = {
  id?: string; name: string; ico?: string; dic?: string; ic_dph?: string;
  street?: string; city?: string; zip?: string; country?: string;
  email?: string; phone?: string; contact_person?: string; notes?: string;
};
...
function CustomersPage() {
  ...
  async function save(c: Customer) {
    ...
    if (c.id) {
      ...
      if (error || !created) { const { friendlyError } = await import("@/lib/faktero/plan-error"); return toast.error(friendlyError(error)); }
      try {
        await triggerEvt({ data: { companyId: cid, event: "customer.created", data: {
          customer_id: created.id, name: created.name, email: created.email, ico: created.ico,
        } } });
      } catch {}
    }
  ...
  async function confirmRow() {
    if (!rowDelete) return;
    try { await list.softDelete([rowDelete.id]); toast.success("Odberateľ vymazaný"); }
  ...
  return (
    <>
      <PageHeader title="Odberatelia" description="Vaši zákazníci a ich fakturačné údaje." action={
      ...
      <PageBody>
        <div className="mb-3 flex flex-wrap items-center justify-between gap-3">
          <input value={list.search} onChange={(e) => list.setSearch(e.target.value)} placeholder="Hľadať názov, IČO, email…"
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="p-3">Názov</th><th className="p-3">IČO</th><th className="p-3">DIČ</th><th className="p-3">Email</th><th className="p-3"></th>
              </tr>
            </thead>
            <tbody className="divide-y divide-border">
              {list.loading && <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">Načítavam…</td></tr>}
              {!list.loading && list.rows.length === 0 && <tr><td colSpan={6} className="p-8 text-center text-muted-foreground">{list.showDeleted ? "Žiadni vymazaní odberatelia." : "Žiadni odberatelia."}</td></tr>}
              {list.rows.map((c) => (
                <tr key={c.id} className="hover:bg-muted/30">
                  <td className="p-3" onClick={(e) => e.stopPropagation()}>
                    <input type="checkbox" checked={!!list.selected[c.id]} onChange={(e) => list.toggleSelect(c.id, e.target.checked)} />
                  </td>
                  ...
                  <td className="p-3 text-right">
                    {!list.showDeleted && <button onClick={() => setEditing(c)} className="rounded p-1.5 hover:bg-muted"><Pencil className="h-4 w-4" /></button>}
                    {list.showDeleted ? (
                      <button onClick={async () => { try { await list.restore([c.id]); toast.success("Obnovené"); } catch (e: any) { toast.error(e?.message ?? "Chyba"); } }} className="rounded p-1.5 text-primary hover:bg-primary/10"><RotateCcw className="h-4 w-4" /></button>
                    ) : (
                      <button onClick={() => setRowDelete(c)} className="rounded p-1.5 text-destructive hover:bg-destructive/10"><Trash2 className="h-4 w-4" /></button>
                    )}
                  </td>
                </tr>
              ))}
      ...
      {editing && <CustomerDialog initial={editing} onClose={() => setEditing(null)} onSave={save} />}
      <ConfirmDialog open={!!rowDelete} title="Naozaj chcete vymazať tohto odberateľa?"
        message={rowDelete ? `${rowDelete.name} bude skrytý z rozhrania. Existujúce faktúry ostanú.` : ""}
        onCancel={() => setRowDelete(null)} onConfirm={confirmRow} />
      <ConfirmDialog open={bulkDelete} title={`Vymazať ${list.selectedIds.length} odberateľov?`}
...
function CustomerDialog({ initial, onClose, onSave }: { initial: Customer; onClose: () => void; onSave: (c: Customer) => void }) {
  ...
  function f<K extends keyof Customer>(k: K, v: any) { setC((p) => ({ ...p, [k]: v })); }
  useEffect(() => {
    const cid = getActiveCompanyId();
  ...
  return (
    <div className="fixed inset-0 z-50 grid place-items-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-2xl rounded-xl border border-border bg-card p-6" onClick={(e) => e.stopPropagation()}>
        ...
        <form onSubmit={(e) => { e.preventDefault(); onSave(c); }} className="mt-4 grid gap-4 sm:grid-cols-2">
          <label className="block sm:col-span-2">
            ...
            <div className="mt-1">
              <CompanyNameAutocomplete
                value={c.name}
                onChange={(v) => f("name", v)}
                onPick={(d, { auto }) => setC((prev) => mergeCompanyAutofill(prev, d, { mode: auto ? "fill-empty" : "overwrite" }) as Customer)}
              />
            </div>
          </label>
          {dup && (
          ...
          <In label="Kontaktná osoba" value={c.contact_person ?? ""} onChange={(v) => f("contact_person", v)} />
          ...
          <label className="sm:col-span-2 block">
            ...
            <textarea value={c.notes ?? ""} onChange={(e) => f("notes", e.target.value)} rows={2} className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" />
          </label>
