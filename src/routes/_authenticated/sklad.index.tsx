  ...
  return (
    <>
      <PageHeader
        ...
        description="Prehľad stavu skladu, položiek a posledných pohybov."
      />
      <PageBody>
        {loading ? (
          ...
          <div className="rounded-xl border border-border bg-card p-8 text-center text-sm text-muted-foreground">
            Žiadne dáta.{" "}
            <Link to="/sklad/nastavenia" className="text-primary hover:underline">
              Vytvorte sklad
            </Link>
            .
          </div>
          ...
          <>
            <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
              ...
              <Stat
                icon={AlertTriangle}
                label="Pod minimom"
                value={String(data.below_min_count)}
                accent={data.below_min_count ? "text-amber-600" : "text-muted-foreground"}
              />
            </div>
            <div className="mt-6 grid gap-4 lg:grid-cols-2">
              <Card title="Položky pod minimálnym stavom">
                {data.below_min_items.length === 0 ? (
                  <p className="text-sm text-muted-foreground">
                    Všetky položky majú dostatočný stav.
                  </p>
                ) : null}
                <Link
                  to="/sklad/produkty"
                ...
                >
                  Spravovať položky <ArrowRight className="h-3.5 w-3.5" />
                </Link>
              </Card>
              <Card title="Posledné pohyby">
                ...
                <Link
                  ...
                  className="mt-3 inline-flex items-center gap-1 text-sm text-primary hover:underline"
                >
                  Všetky pohyby <ArrowRight className="h-3.5 w-3.5" />
                </Link>
              </Card>
            </div>
            <div className="mt-6 grid gap-3 sm:grid-cols-3">
              ...
              <QuickLink to="/sklad/dodacie-listy" label="História skenov DL" />
            </div>
          </>
        )}
      </PageBody>
...
function QuickLink({ to, label }: { to: string; label: string }) {
  return (
    <Link
      to={to as any}
      className="flex items-center justify-between rounded-xl border border-border bg-card px-4 py-3 text-sm font-medium shadow-sm hover:border-primary/40 hover:bg-secondary/40"
    >
