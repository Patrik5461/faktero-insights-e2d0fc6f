...
import {
  getMyPaymentProvider, savePaymentProvider, testPaymentProvider,
  disconnectPaymentProvider, setOnlinePaymentsEnabled,
  rotateWebhookSecret, getPaymentDiagnostics,
} from "@/lib/faktero/payments.functions";
import { toast } from "sonner";
import { Switch } from "@/components/ui/switch";
import { Link } from "@tanstack/react-router";
import { ShieldCheck, ExternalLink, CheckCircle2, AlertCircle, RefreshCw, Copy, KeyRound } from "lucide-react";
...
function OnlinePaymentsPage() {
  ...
  const [diag, setDiag] = useState<{ paymentSecretsKey: boolean; hasWebhookSecret: boolean; notifyUrl: string | null } | null>(null);
  const [newNotifyUrl, setNewNotifyUrl] = useState<string | null>(null);
  const [form, setForm] = useState({ goid: "", client_id: "", client_secret: "", sandbox_mode: true });
  ...
  const statusColor = !state?.provider
    ...
    : sandbox ? "bg-amber-100 text-amber-800" : "bg-emerald-100 text-emerald-800";
  ...
  async function onSave(e: React.FormEvent) {
    ...
    } catch (e: any) { toast.error(e.message ?? "Chyba pri ukladaní."); }
  ...
  async function onTest() {
    if (!companyId) return;
    setBusy("test");
    try { await fnTest({ data: { companyId } }); toast.success("Pripojenie funguje."); await refresh(companyId); }
    catch (e: any) { toast.error(e.message ?? "Test zlyhal."); await refresh(companyId); }
    finally { setBusy(null); }
  }
  ...
  async function onDisconnect() {
    ...
    setBusy("disc");
    try { await fnDisc({ data: { companyId } }); toast.success("Odpojené."); setForm({ goid: "", client_id: "", client_secret: "", sandbox_mode: true }); await refresh(companyId); }
  ...
  async function onToggle(v: boolean) {
    if (!companyId) return;
    try { await fnToggle({ data: { companyId, enabled: v } }); await refresh(companyId); }
    catch (e: any) { toast.error(e.message); }
  }

  async function onRotate() {
    if (!companyId) return;
    if (!confirm("Naozaj pregenerovať webhook secret? Po zmene musíte aktualizovať notification URL v GoPay administrácii, inak prestanú prichádzať notifikácie o platbách.")) return;
    setBusy("rotate");
    try {
      const r = await fnRotate({ data: { companyId } });
      setNewNotifyUrl(r.notifyUrl);
      try { await navigator.clipboard.writeText(r.notifyUrl); } catch {}
      toast.success("Webhook secret pregenerovaný a notification URL skopírovaná.");
      await refresh(companyId);
    } catch (e: any) { toast.error(e.message ?? "Chyba pri rotácii."); }
    finally { setBusy(null); }
  }

  async function copyText(s: string) {
    try { await navigator.clipboard.writeText(s); toast.success("Skopírované."); }
    catch { toast.error("Kopírovanie zlyhalo."); }
  }

  return (
    <>
      <PageHeader title="Online platby" description="Pripojte si vlastný GoPay účet a prijímajte platby priamo od svojich zákazníkov." />
      <PageBody>
        <div className="grid gap-6 lg:grid-cols-3">
            ...
            <div className="rounded-xl border border-border bg-card p-6">
              ...
              {state?.provider?.last_test_at && (
                <div className="mt-4 flex items-start gap-2 text-sm">
                  {state.provider.last_test_ok
                    ? <><CheckCircle2 className="h-4 w-4 text-emerald-600 mt-0.5" /><span>Posledný test prešiel.</span></>
                    : <><AlertCircle className="h-4 w-4 text-destructive mt-0.5" /><span>Posledný test zlyhal: {state.provider.last_test_error}</span></>}
                </div>
              )}
              ...
              <form onSubmit={onSave} className="mt-6 grid gap-4 sm:grid-cols-2">
                <label className="block sm:col-span-2">
                  ...
                  <input value={form.goid} onChange={(e) => setForm({ ...form, goid: e.target.value })}
                ...
                <label className="block">
                  <span className="text-sm font-medium">Client Secret {connected && <span className="text-xs text-muted-foreground">(nechajte prázdne pre zachovanie)</span>}</span>
                ...
                <div className="sm:col-span-2 flex flex-wrap gap-2 pt-2">
                  ...
                  <button type="button" onClick={onTest} disabled={!connected || busy === "test"}
                  ...
                  {connected && (
                    <button type="button" onClick={onDisconnect} disabled={busy === "disc"}
              ...
              <div className="rounded-xl border border-border bg-card p-6">
                <div className="flex items-center justify-between gap-4">
                  <div>
                    ...
                    <p className="text-sm text-muted-foreground">Po zapnutí sa na faktúrach zobrazí tlačidlo „Zaplatiť online".</p>
                  </div>
            ...
            {connected && (
              <div className="rounded-xl border border-border bg-card p-6 space-y-4">
                <div className="flex items-start justify-between gap-4">
                  <div>
                    <h3 className="font-semibold flex items-center gap-2"><KeyRound className="h-4 w-4" /> Webhook secret</h3>
                    <p className="text-sm text-muted-foreground mt-1">
                      Tajný kľúč v notification URL, ktorou GoPay potvrdzuje platby. Pri podozrení na únik ho pregenerujte.
                    </p>
                  </div>
                  <button onClick={onRotate} disabled={busy === "rotate"}
                    ...
                    <RefreshCw className="h-4 w-4" /> {busy === "rotate" ? "Generujem…" : "Pregenerovať webhook secret"}
                  </button>
                </div>
                ...
                {newNotifyUrl && (
                  <div className="rounded-md border border-amber-300 bg-amber-50 p-4 text-sm space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <code className="flex-1 truncate rounded bg-white/70 px-2 py-1 text-xs">{newNotifyUrl}</code>
                      <button onClick={() => copyText(newNotifyUrl)} className="inline-flex items-center gap-1 rounded-md border border-amber-300 px-2 py-1 text-xs hover:bg-amber-100">
                ...
                {!newNotifyUrl && diag?.notifyUrl && (
                  <div className="rounded-md border border-border bg-secondary/40 p-4 text-sm space-y-2">
                    ...
                    <div className="flex items-center gap-2">
                      <code className="flex-1 truncate rounded bg-background px-2 py-1 text-xs">{diag.notifyUrl}</code>
                      <button onClick={() => copyText(diag.notifyUrl!)} className="inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs hover:bg-secondary">
                        <Copy className="h-3.5 w-3.5" /> Kopírovať
                      </button>
                    </div>
                  </div>
                ...
                {diag && !diag.paymentSecretsKey && (
                  <div className="rounded-md border border-amber-300 bg-amber-50 p-3 text-xs text-amber-900 flex items-start gap-2">
                    ...
                    <span><strong>PAYMENT_SECRETS_KEY nie je nastavený.</strong> Šifrovanie GoPay kľúčov používa záložný kľúč. Pre produkciu nastavte vyhradený PAYMENT_SECRETS_KEY na serveri.</span>
                  </div>
                )}
              </div>
            )}
          ...
          <aside className="space-y-4">
            <div className="rounded-xl border border-emerald-200 bg-emerald-50/50 p-5">
              <div className="flex items-start gap-3">
                ...
                <div className="text-sm text-emerald-900 space-y-2">
                  <p><strong>Peniaze nikdy nejdú cez Faktero.</strong></p>
                  <p>Zákazník platí priamo na váš GoPay účet. Faktero si neúčtuje províziu z platieb vašich zákazníkov.</p>
                </div>
              </div>
            </div>
            <div className="rounded-xl border border-border bg-card p-5 text-sm space-y-2">
              ...
              <p className="text-muted-foreground">Pozrite si návod ako získať GoPay účet a pripojiť ho k Faktere.</p>
              <Link to="/docs/online-platby/gopay" className="inline-flex items-center gap-1 text-primary hover:underline">
