...
function NewRecurring() {
  ...
  useEffect(() => {
    ...
    if (!cid) return;
    supabase.from("customers").select("id, name, ico, dic, ic_dph, street, city, zip, country, email")
      .eq("company_id", cid).then(({ data }) => setCustomers(data ?? []));
  }, []);
  ...
  async function submit(e: React.FormEvent) {
    ...
    const { data, error } = await supabase.from("recurring_invoices").insert({
      ...
      frequency: form.frequency, next_run: form.next_run, currency: form.currency, due_days: form.due_days,
      payment_method: form.payment_method, notes: form.notes, active: form.active,
      items: items as any,
      subtotal: totals.subtotal, vat_total: totals.vat_total, total: totals.total,
    }).select().single();
    if (error || !data) { const { friendlyError } = await import("@/lib/faktero/plan-error"); return toast.error(friendlyError(error)); }
  ...
  return (
    <>
      <PageHeader title="Nová opakovaná faktúra" description="Šablóna, podľa ktorej bude Faktero pravidelne vystavovať faktúry." />
      <PageBody>
        <form onSubmit={submit} className="space-y-6">
          <div className="grid gap-4 rounded-xl border border-border bg-card p-5 sm:grid-cols-3">
            <label className="block sm:col-span-3">
              ...
              <input required value={form.name} onChange={e => setForm({ ...form, name: e.target.value })} placeholder="napr. Mesačný paušál ACME"
            ...
            <label className="block sm:col-span-3">
              ...
              <div className="mt-1 flex gap-2">
                ...
                <button type="button" onClick={() => setNewCustOpen(true)} className="inline-flex shrink-0 items-center gap-1.5 rounded-md border border-border px-3 text-sm hover:bg-secondary">
            ...
            <label className="block">
              ...
              <input type="number" min={1} value={form.due_days} onChange={e => setForm({ ...form, due_days: Number(e.target.value) })}
                className="mt-1 w-full rounded-md border border-input bg-background px-3 py-2 text-sm" />
            </label>
            <label className="block">
          ...
          <div className="rounded-xl border border-border bg-card p-5">
            ...
            <div className="space-y-3">
              {items.map((it, idx) => (
                <div key={idx} className="grid gap-2 rounded-md border border-border p-3 sm:grid-cols-[2fr_80px_80px_100px_80px_120px_auto] sm:items-end">
                  ...
                  <div className="text-right text-sm font-medium">{(it.quantity * it.unit_price * (1 + it.vat_rate / 100)).toFixed(2)} {form.currency}</div>
            ...
            <div className="mt-4 space-y-1 text-right text-sm">
              <div>Bez DPH: <span className="font-medium">{totals.subtotal.toFixed(2)} {form.currency}</span></div>
              <div>DPH: <span className="font-medium">{totals.vat_total.toFixed(2)} {form.currency}</span></div>
              <div className="text-lg font-semibold">Spolu: {totals.total.toFixed(2)} {form.currency}</div>
            </div>
          </div>
          ...
          <div className="flex justify-end">
            <button type="submit" className="rounded-md bg-primary px-5 py-2.5 text-sm font-medium text-primary-foreground hover:opacity-90">
              Vytvoriť šablónu
            </button>
          </div>
        </form>
