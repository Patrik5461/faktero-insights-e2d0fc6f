    ...
    (async () => {
      ...
      if (ids.length) {
        ...
        const prodIds = Array.from(
          new Set((si ?? []).map((x: any) => x.product_id).filter(Boolean)),
        );
        if (prodIds.length) {
      ...
      const { data: whAll } = await supabase
        .from("warehouses")
        .select("id, name")
        .eq("company_id", cid)
        .order("created_at");
      ...
      if (invIds.length) {
        ...
        (inv ?? []).forEach((i: any) => {
          im[i.id] = { id: i.id, number: i.invoice_number };
        });
        setInvoices(im);
      } else setInvoices({});
      setLoading(false);
  ...
  return (
    <>
      <PageHeader
        ...
        description="História všetkých pohybov skladu."
        action={
      ...
      <PageBody>
        <div className="mb-3 flex flex-wrap items-center gap-3">
          ...
          <input
            ...
            placeholder="Hľadať produkt / SKU…"
          ...
          {stock_item_id && (
            <div className="inline-flex items-center gap-2 rounded-md border border-primary/30 bg-primary/5 px-3 py-1.5 text-xs">
              ...
              <Link to="/sklad/pohyby" className="text-primary hover:underline">
                × zrušiť
              </Link>
            </div>
          )}
          ...
          <table className="w-full text-sm">
            <thead className="bg-muted/40 text-left text-xs uppercase tracking-wide text-muted-foreground">
              <tr>
                ...
                <th className="p-3 text-right">Hodnota</th>
            ...
            <tbody className="divide-y divide-border">
              ...
              {visibleRows.map((m) => (
                <tr
                  ...
                  onClick={() => navigate({ to: "/sklad/pohyby/$id", params: { id: m.id } })}
                >
                  ...
                  <td className="p-3">
                    {items[m.stock_item_id]?.sku ?? m.stock_item_id.slice(0, 8)}
                  </td>
                  ...
                  <td className="p-3 text-xs" onClick={(e) => e.stopPropagation()}>
                    ...
                    invoices[m.source_document_id ?? m.reference_id] ? (
                      <Link
                        to="/faktury/$id"
                        params={{ id: invoices[m.source_document_id ?? m.reference_id].id }}
                      ...
                      <span className="inline-flex items-center rounded-full bg-stone-100 px-2 py-0.5 text-[11px] font-medium text-stone-700">
                        ...
                        )[m.source_document_type] ?? m.source_document_type}
                      </span>
                    ) : (
        ...
        <div className="md:hidden space-y-2">
          ...
          {visibleRows.map((m) => (
            <Link
              ...
              to="/sklad/pohyby/$id"
              params={{ id: m.id }}
              className="block rounded-xl border border-border bg-card p-3 hover:bg-muted/30"
            >
