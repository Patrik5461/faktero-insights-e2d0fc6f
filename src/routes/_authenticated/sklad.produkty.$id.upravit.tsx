...
function EditStockProduct() {
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      setCategories(cats ?? []);
      setSuppliers(sups ?? []);
      const p = detail.product; const si = detail.stockItem;
    ...
    }).catch((e) => setError(e?.message ?? "Chyba načítania")).finally(() => setLoading(false));
  ...
  async function handlePhoto(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    try {
      ...
      const { error: upErr } = await supabase.storage.from("product-photos").upload(path, file, { upsert: true, contentType: file.type });
      if (upErr) throw upErr;
      set("photo_url", path);
      const { data: signed } = await supabase.storage.from("product-photos").createSignedUrl(path, 60 * 60);
  ...
  async function handleAddCategory() {
    ...
    try {
      const created: any = await addCategory({ data: { company_id: cid, name: newCategoryName.trim() } });
  ...
  if (!canManage) return <PageBody><div className="text-sm text-muted-foreground">Nemáte oprávnenie upravovať skladové karty.</div></PageBody>;

  return (
    <>
      <PageHeader title="Upraviť skladovú kartu" description={form.name || "Produkt"} action={
        <div className="flex gap-2">
          <Link to="/sklad/produkty/$id" params={{ id }} className="inline-flex items-center gap-1 rounded-md border border-border bg-card px-3 py-2 text-sm hover:bg-secondary">
      ...
      <PageBody>
        <form onSubmit={handleSave} className="space-y-4">
            ...
            <div className="rounded-xl border border-border bg-card p-3">
              ...
              {photoPreview ? (
                <div className="relative">
                  <img src={photoPreview} alt="produkt" className="aspect-square w-full rounded-md object-cover" />
                  <button type="button" onClick={() => { set("photo_url", ""); setPhotoPreview(null); }}
            ...
            <div className="rounded-xl border border-border bg-card p-4">
              ...
              <div className="grid gap-3 sm:grid-cols-2">
                ...
                <Field label="Názov (EN)">
                  <input value={form.name_en} onChange={(e) => set("name_en", e.target.value)} className="input" />
                </Field>
                <Field label="Kód">
                ...
                <Field label="Čiarový kód / EAN">
                  <input value={form.barcode} onChange={(e) => set("barcode", e.target.value)} className="input" placeholder="napr. 8588001234567" />
                </Field>
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <Field label="Nákupná cena (€)">
                <input type="number" step="0.0001" min="0" value={form.purchase_price} onChange={(e) => set("purchase_price", Number(e.target.value))} className="input" />
              </Field>
              <Field label="Predajná cena (€)">
                <input type="number" step="0.01" min="0" value={form.sale_price} onChange={(e) => set("sale_price", Number(e.target.value))} className="input" />
              </Field>
              <Field label="DPH sadzba (%)">
                <select value={form.vat_rate} onChange={(e) => set("vat_rate", Number(e.target.value))} className="input">
            ...
            {form.sale_price > 0 && (
              <p className="mt-2 text-xs text-muted-foreground">
                S DPH: {(form.sale_price * (1 + form.vat_rate / 100)).toFixed(2)} € · Marža: {form.purchase_price > 0 ? (((form.sale_price - form.purchase_price) / form.purchase_price) * 100).toFixed(1) : "—"} %
              </p>
            )}
          </div>
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-2">
              <Field label="Kategória">
                <div className="flex gap-2">
                  <select value={form.category_id} onChange={(e) => set("category_id", e.target.value)} className="input flex-1">
                    <option value="">— bez kategórie —</option>
                    {categories.map((c) => <option key={c.id} value={c.id}>{c.name}</option>)}
                  </select>
                </div>
                <div className="mt-2 flex gap-2">
                  <input value={newCategoryName} onChange={(e) => setNewCategoryName(e.target.value)} placeholder="Nová kategória…" className="input flex-1" />
                  <button type="button" onClick={handleAddCategory} className="rounded-md border border-border bg-secondary px-3 py-2 text-xs hover:bg-secondary/70">Pridať</button>
                </div>
              </Field>
              <Field label="Dodávateľ">
                <select value={form.supplier_id} onChange={(e) => set("supplier_id", e.target.value)} className="input">
                  ...
                  {suppliers.map((s) => <option key={s.id} value={s.id}>{s.name}{s.ico ? ` (${s.ico})` : ""}</option>)}
                </select>
                <p className="mt-1 text-[11px] text-muted-foreground">Dodávatelia sa spravujú v Zákazníci.</p>
              </Field>
            </div>
          </div>
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            ...
            <div className="grid gap-3 sm:grid-cols-3">
              <Field label="Minimálna zásoba">
                <input type="number" step="0.01" min="0" value={form.min_stock} onChange={(e) => set("min_stock", Number(e.target.value))} className="input" />
              </Field>
              <Field label="Lokácia (regál/pozícia)">
                <input value={form.location} onChange={(e) => set("location", e.target.value)} className="input" placeholder="napr. A-3-12" />
              </Field>
          ...
          <div className="sticky bottom-0 flex flex-wrap items-center justify-end gap-2 border-t border-border bg-background/95 py-3 backdrop-blur">
            <Link to="/sklad/produkty/$id" params={{ id }} className="rounded-md border border-border bg-card px-4 py-2 text-sm hover:bg-secondary">Zrušiť</Link>
            <button type="submit" disabled={saving} className="inline-flex items-center gap-1 rounded-md bg-primary px-4 py-2 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-50">
