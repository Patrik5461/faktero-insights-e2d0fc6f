...
function CategoriesPage() {
  ...
  function resetForm() { setEditing(null); setName(""); setColor(""); setNote(""); }
  ...
  function startEdit(r: any) {
    setEditing(r); setName(r.name); setColor(r.color ?? ""); setNote(r.note ?? "");
  }

  return (
    <>
      <PageHeader title="Kategórie skladu" description="Rozdeľte produkty do kategórií pre lepší prehľad a filtrovanie." />
      <PageBody>
        <div className="grid gap-6 lg:grid-cols-[1fr_20rem]">
          <div className="overflow-hidden rounded-xl border border-border bg-card">
            <table className="w-full text-sm">
              ...
              <tbody className="divide-y divide-border">
                ...
                {!loading && rows.length === 0 && <tr><td colSpan={4} className="p-8 text-center text-muted-foreground"><Tags className="mx-auto mb-2 h-6 w-6 opacity-40" />Žiadne kategórie. Vytvorte prvú vpravo.</td></tr>}
                {rows.map((r) => (
                  <tr key={r.id}>
                    <td className="p-3">
                    ...
                    <td className="p-3 text-right">
                      <div className="inline-flex gap-1">
                        <button onClick={() => startEdit(r)} className="rounded-md border border-border p-1.5 hover:bg-muted" title="Upraviť"><Pencil className="h-3.5 w-3.5" /></button>
                        <button onClick={() => onDelete(r.id, r.product_count)} className="rounded-md border border-border p-1.5 text-destructive hover:bg-destructive/10" title="Zmazať"><Trash2 className="h-3.5 w-3.5" /></button>
                      </div>
                    </td>
                  </tr>
                ))}
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            <div className="mb-3 text-sm font-semibold">{editing ? "Upraviť kategóriu" : "Nová kategória"}</div>
            ...
            <div className="mt-1 flex flex-wrap gap-1.5">
              <button onClick={() => setColor("")} className={`h-7 w-7 rounded-full border ${color === "" ? "ring-2 ring-primary" : "border-border"}`} title="Bez farby" />
              {PALETTE.map((c) => (
                <button key={c} onClick={() => setColor(c)} className={`h-7 w-7 rounded-full border border-border ${color === c ? "ring-2 ring-primary" : ""}`} style={{ background: c }} />
              ))}
            </div>
            ...
            <div className="mt-4 flex gap-2">
              <button disabled={busy || !name.trim()} onClick={onSave} className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-2 text-sm font-medium text-primary-foreground hover:opacity-90 disabled:opacity-60">
                <Plus className="h-3.5 w-3.5" />{editing ? "Uložiť" : "Vytvoriť"}
              </button>
              {editing && <button onClick={resetForm} className="rounded-md border border-border px-3 py-2 text-sm hover:bg-muted">Zrušiť</button>}
            </div>
          </div>
        </div>
      </PageBody>
