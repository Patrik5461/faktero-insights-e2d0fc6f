...
function ProductStockDetail() {
  ...
  function exportMovementsCsv() {
    ...
    a.href = url;
    a.download = `pohyby-${data.product?.code ?? id}-${new Date().toISOString().slice(0, 10)}.csv`;
    a.click();
  ...
  if (!data?.product)
    return (
      <PageBody>
        <div className="text-sm text-muted-foreground">Produkt sa nenašiel.</div>
      </PageBody>
    );
    ...
    <>
      ...
      <PageBody>
        <div className="grid gap-4 md:grid-cols-4">
          ...
          <Stat
            label="K dispozícii"
            value={`${(Number(data.totalQuantity) - Number(data.reservedQuantity ?? 0)).toFixed(2)} ${si?.unit ?? ""}`}
          />
          <Stat label="Hodnota skladu" value={`${stockValue.toFixed(2)} €`} />
          <Stat
            label="Minimálny stav"
            value={si ? `${Number(si.min_stock).toFixed(2)} ${si.unit}` : "—"}
          />
          <Stat
            label="Nákupná cena"
        ...
        <div className="mt-4 grid gap-4 md:grid-cols-[220px_1fr]">
          ...
          <div className="rounded-xl border border-border bg-card p-4">
            <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
              <Package className="h-4 w-4 text-primary" /> Detaily produktu
            </div>
        ...
        <div className="mt-4">
          <ReservationsPanel
            companyId={data.stockItem?.company_id ?? p.company_id}
            stockItemId={si?.id ?? ""}
            unit={si?.unit}
          />
        </div>
        ...
        <div className="mt-4 rounded-xl border border-border bg-card p-4">
          <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
            <Warehouse className="h-4 w-4 text-primary" /> Stav v skladoch
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="text-left text-xs uppercase tracking-wide text-muted-foreground">
                <tr>
                  ...
                  <th className="p-2 text-right">Rezervované</th>
                </tr>
              </thead>
                ...
                {data.levels.map((l: any) => (
                  <tr key={l.warehouse_id}>
                    ...
                    <td className="p-2 text-right tabular-nums text-muted-foreground">
                      {Number(l.reserved_quantity ?? 0).toFixed(2)}
                    </td>
                  </tr>
                ))}
              </tbody>
        ...
        <div className="mt-4 rounded-xl border border-border bg-card p-4">
          <div className="mb-3 flex items-center gap-2 text-sm font-semibold">
            <FileText className="h-4 w-4 text-primary" /> Posledných 20 pohybov
          </div>
            ...
            <table className="w-full text-sm">
              <thead className="text-left text-xs uppercase tracking-wide text-muted-foreground">
                <tr>
                  ...
                  <th className="p-2 text-right">Množstvo</th>
                  <th className="p-2 text-right">Cena/MJ</th>
                  <th className="p-2 text-right">Hodnota</th>
              ...
              <tbody className="divide-y divide-border">
                {data.movements.map((m: any) => (
                  <tr key={m.id}>
                    ...
                    <td className="p-2">
                      <SourceBadge type={m.source_document_type} id={m.source_document_id} />
                    </td>
