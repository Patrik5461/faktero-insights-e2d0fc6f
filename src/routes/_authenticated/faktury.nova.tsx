...
function NewInvoice() {
  ...
  useEffect(() => {
    ...
    if (!cid) return;
    supabase.from("customers").select("id, name, ico, dic, ic_dph, street, city, zip, country, email").eq("company_id", cid).order("name").then(({ data }) => setCustomers(data ?? []));
    supabase.from("products").select("id, name, unit, unit_price, vat_rate, description").eq("company_id", cid).eq("active", true).order("name").then(({ data }) => setProducts(data ?? []));
    supabase.from("companies").select("*").eq("id", cid).single().then(({ data }) => { if (data) { setCompany(data); setForm((f) => ({ ...f, currency: data.default_currency || "EUR" })); } });
    // Build product_id → stock metadata map
    (async () => {
      const [{ data: si }, { data: lvl }, { data: wh }] = await Promise.all([
        supabase.from("stock_items").select("id, product_id, sku, track_stock").eq("company_id", cid),
        supabase.from("stock_levels").select("stock_item_id, quantity").eq("company_id", cid),
        supabase.from("warehouses").select("id, name").eq("company_id", cid).eq("active", true).order("created_at").limit(1),
      ]);
      const totals = new Map<string, number>();
      (lvl ?? []).forEach((l: any) => totals.set(l.stock_item_id, (totals.get(l.stock_item_id) ?? 0) + Number(l.quantity)));
      const m: Record<string, StockMeta> = {};
      (si ?? []).forEach((s: any) => {
        if (!s.product_id) return;
        m[s.product_id] = { stock_item_id: s.id, track_stock: !!s.track_stock, available: totals.get(s.id) ?? 0, sku: s.sku ?? null };
      });
      setStockByProduct(m);
  ...
  useEffect(() => {
    function onKey(e: KeyboardEvent) {
      ...
      if (!mod) return;
      if (e.key === "Enter") { e.preventDefault(); document.getElementById("invoice-submit")?.click(); }
      else if (e.key.toLowerCase() === "k") { e.preventDefault(); setAiOpen(true); }
      else if (e.key.toLowerCase() === "i") { e.preventDefault(); setItems((arr) => [...arr, { ...EMPTY_ITEM }]); }
    }
  ...
  async function runAi() {
    ...
    try {
      ...
      if (r.customer_hint) {
        const match = customers.find((c) => c.name.toLowerCase().includes(r.customer_hint!.toLowerCase()));
  ...
  async function submit(e: React.FormEvent) {
    ...
    if (offending) {
      toast.error(`Na sklade nie je dostatok kusov pre „${offending.name}". Dostupné: ${offending._available ?? 0}.`);
      return;
    }
    setSubmitting(true);
    ...
    if (error || !inv) { const { friendlyError } = await import("@/lib/faktero/plan-error"); toast.error(friendlyError(error)); setSubmitting(false); return; }
    ...
    try {
      await triggerEvt({ data: { companyId: cid, event: "invoice.created", data: {
        invoice_id: inv.id, invoice_number: inv.invoice_number, status: inv.status, total: Number(inv.total), currency: inv.currency, external_id: inv.external_id ?? null, customer_id: inv.customer_id ?? null, customer_name: inv.customer_name, customer_email: inv.customer_email,
      } } });
    } catch {}
      ...
      <PageHeader
        ...
        action={
          ...
          >
            ...
            <kbd className="ml-1 hidden rounded bg-primary/10 px-1.5 py-0.5 text-[10px] font-mono sm:inline">⌘K</kbd>
          </button>
        }
      />
      <PageBody>
        <form onSubmit={submit} className="mx-auto max-w-5xl space-y-6">
                  ...
                  <div className="flex gap-1">
                    {[7, 14, 30].map((d) => (
                      <button key={d} type="button"
                        onClick={() => setForm({ ...form, due_date: new Date(Date.now() + d * 86400000).toISOString().slice(0, 10) })}
          ...
          <section className="rounded-2xl border border-border bg-card p-5">
            <SectionHeader icon={Package} title="Položky">
              <ProductSearch products={products} onPick={(p) => {
                setItems((arr) => {
                  const next = [...arr];
                  ...
                  {items.map((it, idx) => (
                    <tr key={idx} className="border-b border-border/60">
                      <td className="py-2 pr-3">
                        <input value={it.name} onChange={(e) => setItem(idx, { name: e.target.value })} placeholder="Názov položky"
                      ...
                      <td className="py-2 pl-3"><CellNum value={it.quantity} onChange={(v) => setItem(idx, { quantity: v })} w="w-16" /></td>
            ...
            <button type="button" onClick={() => setItems([...items, { ...EMPTY_ITEM }])}
              ...
              <kbd className="ml-1 hidden rounded bg-muted px-1.5 py-0.5 text-[10px] font-mono sm:inline">⌘I</kbd>
            </button>
          </section>
          ...
          <div className="sticky bottom-4 z-10 flex flex-col-reverse gap-2 rounded-2xl border border-border bg-card/95 p-4 backdrop-blur sm:flex-row sm:items-center sm:justify-between">
            ...
            <div className="flex gap-2">
              <button type="button" onClick={() => navigate({ to: "/faktury" })}
                className="rounded-md border border-border bg-card px-4 py-2 text-sm font-medium hover:bg-secondary">
                Zrušiť
              </button>
              <button id="invoice-submit" type="submit" disabled={submitting}
...
function CellNum({ value, onChange, w = "w-20", align = "left" }: { value: number; onChange: (v: number) => void; w?: string; align?: "left" | "right" }) {
  return (
    <input
...
function CustomerSearch({ customers, value, onChange, onCreated }: { customers: any[]; value: string; onChange: (id: string) => void; onCreated: (c: any) => void }) {
  ...
  const filtered = q
    ? customers.filter((c) => `${c.name} ${c.ico ?? ""} ${c.email ?? ""}`.toLowerCase().includes(q.toLowerCase())).slice(0, 8)
    : customers.slice(0, 8);
    ...
    <div ref={ref} className="relative mt-1">
      ...
      {open && (
        <div className="absolute left-0 right-0 top-full z-20 mt-1 rounded-lg border border-border bg-card shadow-lg">
          <div className="flex items-center gap-2 border-b border-border px-3 py-2">
            ...
            <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Hľadať podľa mena, IČO, e-mailu…"
          ...
          <ul className="max-h-64 overflow-auto py-1">
            ...
            {filtered.map((c) => (
              <li key={c.id}>
                <button type="button" onClick={() => { onChange(c.id); setOpen(false); setQ(""); }}
                  className="flex w-full flex-col items-start px-3 py-2 text-left text-sm hover:bg-muted/60">
                  <span className="font-medium">{c.name}</span>
                  <span className="text-xs text-muted-foreground">{[c.ico, c.email].filter(Boolean).join(" · ")}</span>
                </button>
              </li>
            ))}
          </ul>
      ...
      {modalOpen && (
        <NewCustomerModal
          ...
          onCreated={(c) => { onCreated(c); setModalOpen(false); setQ(""); }}
        />
      )}
    </div>
  );
...
function NewCustomerModal({ defaultName, onClose, onCreated }: { defaultName?: string; onClose: () => void; onCreated: (c: any) => void }) {
        ...
        <form onSubmit={save} className="space-y-4 p-5">
          {dup && (
            <div className="rounded-md border border-amber-300/50 bg-amber-50 px-3 py-2 text-xs text-amber-900 dark:border-amber-700/40 dark:bg-amber-950/40 dark:text-amber-200">
              ...
              <button type="button" onClick={() => onCreated(dup)} className="ml-2 underline hover:no-underline">Použiť existujúceho</button>
            </div>
          )}
          <div className="grid gap-3 sm:grid-cols-2">
            ...
            <Field label="IČO *">
              <div className="flex -space-x-px items-start">
                <input value={f.ico} onChange={(e) => setF({ ...f, ico: e.target.value })} className={`${modalInput} rounded-l-md focus:z-10`} />
                <IcoLookupButton
                  ico={f.ico}
            ...
            <Field label="DIČ"><input value={f.dic} onChange={(e) => setF({ ...f, dic: e.target.value })} className={modalInput} /></Field>
            <Field label="IČ DPH"><input value={f.ic_dph} onChange={(e) => setF({ ...f, ic_dph: e.target.value })} className={modalInput} /></Field>
            <Field label="Email"><input type="email" value={f.email} onChange={(e) => setF({ ...f, email: e.target.value })} className={modalInput} /></Field>
            <Field label="Telefón"><input value={f.phone} onChange={(e) => setF({ ...f, phone: e.target.value })} className={modalInput} /></Field>
            <Field label="Ulica" className="sm:col-span-2"><input value={f.street} onChange={(e) => setF({ ...f, street: e.target.value })} className={modalInput} /></Field>
            <Field label="Mesto"><input value={f.city} onChange={(e) => setF({ ...f, city: e.target.value })} className={modalInput} /></Field>
            <Field label="PSČ"><input value={f.zip} onChange={(e) => setF({ ...f, zip: e.target.value })} className={modalInput} /></Field>
            <Field label="Krajina"><input value={f.country} onChange={(e) => setF({ ...f, country: e.target.value })} className={modalInput} /></Field>
          </div>
...
const modalInput = "w-full rounded-md border border-input bg-background px-3 py-2 text-sm focus:border-primary focus:outline-none";
...
function ProductSearch({ products, onPick, stockByProduct }: {
  products: any[];
  onPick: (p: any) => void;
    ...
    <div ref={ref} className="relative">
      ...
      {open && (
        <div className="absolute right-0 top-full z-20 mt-1 w-80 rounded-lg border border-border bg-card shadow-lg">
          <div className="flex items-center gap-2 border-b border-border px-3 py-2">
            <Search className="h-4 w-4 text-muted-foreground" />
            <input autoFocus value={q} onChange={(e) => setQ(e.target.value)} placeholder="Hľadať produkt…"
              className="flex-1 bg-transparent text-sm outline-none" />
          </div>
          <ul className="max-h-64 overflow-auto py-1">
            {filtered.length === 0 && <li className="px-3 py-2 text-sm text-muted-foreground">Žiadne produkty</li>}
            {filtered.map((p) => (
              <li key={p.id}>
                <button type="button" onClick={() => { onPick(p); setOpen(false); setQ(""); }}
                  ...
                  <span className="flex flex-col">
                    ...
                    {stockByProduct[p.id]?.track_stock && (
                      <span className={`text-[11px] ${stockByProduct[p.id].available <= 0 ? "text-destructive" : "text-muted-foreground"}`}>
                        Sklad: {stockByProduct[p.id].available} {p.unit ?? "ks"}
                      </span>
                    )}
                  </span>
                  <span className="text-xs text-muted-foreground tabular-nums">{Number(p.unit_price).toFixed(2)} €</span>
                </button>
              </li>
            ))}
          </ul>
...
function StockHint({ item, warehouseName }: { item: Item; warehouseName: string }) {
  ...
  return (
    <div className={`mt-1 inline-flex items-center gap-1 text-[11px] ${insufficient ? "text-destructive" : "text-muted-foreground"}`}>
      {insufficient && <AlertTriangle className="h-3 w-3" />}
      {insufficient
        ? <>Na sklade nie je dostatok kusov. Dostupné: {available}{warehouseName ? ` · ${warehouseName}` : ""}</>
        : <>Sklad: {available} {item.unit}{warehouseName ? ` · ${warehouseName}` : ""}{item._sku ? ` · ${item._sku}` : ""}</>}
    </div>
  );
}

function AiModal({ prompt, setPrompt, loading, onClose, onRun }: {
  prompt: string; setPrompt: (v: string) => void; loading: boolean; onClose: () => void; onRun: () => void;
}) {
  return (
    <div className="fixed inset-0 z-50 flex items-start justify-center bg-background/60 p-4 pt-24 backdrop-blur" onClick={onClose}>
      <div className="w-full max-w-xl rounded-2xl border border-border bg-card p-5 shadow-2xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-3 flex items-center justify-between">
          <span className="text-xs text-muted-foreground"><Command className="inline h-3 w-3" />↵ spustiť</span>
          <div className="flex gap-2">
            <button type="button" onClick={onClose} className="rounded-md border border-border px-3 py-1.5 text-sm hover:bg-secondary">Zrušiť</button>
            <button type="button" onClick={onRun} disabled={loading || !prompt.trim()}
