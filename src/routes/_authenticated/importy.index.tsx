...
function ImportHistoryPage() {
  ...
  const {
    rows: jobs, total, loading, page, setPage, pageSize, setPageSize,
    search, setSearch,
  } = usePagedLogs({
  ...
  async function toggleLogs(jobId: string) {
    ...
    setOpenId(jobId);
    if (!logs[jobId]) {
      const { data } = await supabase.from("import_logs").select("*").eq("import_job_id", jobId).order("row_number").limit(200);
      setLogs((p) => ({ ...p, [jobId]: data ?? [] }));
    }
  }

  async function downloadOriginal(jobId: string) {
    try { const r = await getUrl({ data: { jobId } }); window.open(r.signedUrl, "_blank"); }
  ...
  return (
    <>
      <PageHeader
        ...
        action={
          <Link to="/importy/superfaktura" className="inline-flex items-center gap-1.5 rounded-md bg-primary px-3 py-1.5 text-sm font-medium text-primary-foreground hover:opacity-90">
        ...
        <div className="mx-auto max-w-5xl">
          <div className="mb-4">
            <LogsToolbar
              ...
              selects={[
                {
                  label: "Stav", value: status, onChange: setStatus,
                  options: [
                ...
                {
                  label: "Zdroj", value: source, onChange: setSource,
                  options: [
                    { value: "all", label: "Všetky" },
              ...
              onReset={() => { setSearch(""); setStatus("all"); setSource("all"); setDateFrom(""); setDateTo(""); }}
            />
          </div>
                  ...
                  {jobs.map((j) => (
                    <React.Fragment key={j.id}>
                      <tr className="border-t border-border">
                        <td className="px-4 py-2 whitespace-nowrap">{new Date(j.created_at).toLocaleString("sk-SK")}</td>
                        <td className="px-4 py-2">{j.source}</td>
                        <td className="px-4 py-2 max-w-[240px] truncate" title={j.file_name}>{j.file_name ?? "—"}</td>
                        ...
                        <td className="px-4 py-2 text-right tabular-nums">{j.imported_customers}</td>
                        <td className="px-4 py-2 text-right tabular-nums">{j.failed_rows}</td>
                        <td className="px-4 py-2 text-right">
                          <button onClick={() => downloadOriginal(j.id)} title="Stiahnuť pôvodný súbor" className="rounded-md p-1.5 hover:bg-secondary">
                            <Download className="h-4 w-4" />
                          </button>
                          <button onClick={() => toggleLogs(j.id)} title="Zobraziť logy" className="rounded-md p-1.5 hover:bg-secondary">
                            ...
                            <div className="max-h-72 overflow-auto rounded-md border border-border bg-background">
                              <table className="w-full text-xs">
                                <thead className="text-muted-foreground">
                                  <tr><th className="px-2 py-1 text-left">#</th><th className="px-2 py-1 text-left">Typ</th><th className="px-2 py-1 text-left">Stav</th><th className="px-2 py-1 text-left">Správa</th></tr>
                                </thead>
                                <tbody>
                                  {(logs[j.id] ?? []).map((l) => (
                                  ...
                                  {(logs[j.id] ?? []).length === 0 && (
                                    <tr><td colSpan={4} className="px-2 py-3 text-center text-muted-foreground">Žiadne logy.</td></tr>
                                  )}
                                </tbody>
                              </table>
                            </div>
...
function StatusPill({ status }: { status: string }) {
  ...
  return <span className={`inline-flex rounded-md px-2 py-0.5 text-xs font-medium ${map[status] ?? "bg-secondary"}`}>{status}</span>;
}