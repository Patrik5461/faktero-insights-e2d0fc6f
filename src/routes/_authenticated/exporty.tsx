...
function ExportsPage() {
  ...
  async function load() {
    ...
    const [{ data: js }, { data: inv }] = await Promise.all([
      supabase.from("export_jobs").select("*").eq("company_id", cid).order("created_at", { ascending: false }).limit(50),
      supabase.from("invoices").select("id, invoice_number, customer_name, issue_date, total, currency, status")
        .eq("company_id", cid)
  ...
  const selectedIds = Object.entries(picked).filter(([, v]) => v).map(([k]) => k);
  ...
  async function runExport() {
    ...
    setBusy(true);
    try {
      const r = await exportFn({ data: { companyId: cid, invoiceIds: selectedIds, format: "pohoda_xml" } });
      ...
      <PageBody>
        <div className="grid gap-6 lg:grid-cols-[1fr_360px]">
          ...
          <div className="space-y-4">
            <div className="rounded-2xl border border-border bg-card p-5">
              <div className="mb-4 flex flex-wrap items-end gap-3">
                ...
                <div className="ml-auto flex gap-2">
                  <button onClick={() => setPicked(Object.fromEntries(invoices.map((i) => [i.id, true])))}
                    className="rounded-md border border-border px-3 py-2 text-sm hover:bg-secondary">Vybrať všetko</button>
                  <button onClick={() => setPicked({})}
                    className="rounded-md border border-border px-3 py-2 text-sm hover:bg-secondary">Zrušiť výber</button>
                </div>
              </div>
              ...
              <div className="overflow-hidden rounded-lg border border-border">
                <table className="w-full text-sm">
                  ...
                  <tbody className="divide-y divide-border">
                    {invoices.length === 0 && (
                      <tr><td colSpan={5} className="p-6 text-center text-sm text-muted-foreground">Žiadne faktúry v zvolenom období.</td></tr>
                    )}
                    {invoices.map((i) => (
                      <tr key={i.id} className="hover:bg-muted/30">
                        <td className="p-3">
                          <input type="checkbox" checked={!!picked[i.id]} onChange={(e) => setPicked({ ...picked, [i.id]: e.target.checked })} />
                        </td>
                        <td className="p-3 font-medium">{i.invoice_number}</td>
                        <td className="p-3">{i.customer_name ?? "—"}</td>
                        <td className="p-3">{i.issue_date}</td>
                        <td className="p-3 text-right tabular-nums">{Number(i.total).toFixed(2)} {i.currency}</td>
                      </tr>
                    ))}
                  </tbody>
                </table>
              ...
              <div className="mt-4 flex flex-wrap items-center justify-between gap-3">
                <div className="text-sm text-muted-foreground">Vybraných: <span className="font-semibold text-foreground">{selectedIds.length}</span></div>
                <button onClick={runExport} disabled={busy || !selectedIds.length}
            ...
            {/* History */}
            <div className="rounded-2xl border border-border bg-card p-5">
              <h3 className="mb-3 text-sm font-semibold uppercase tracking-wide">História exportov</h3>
              {jobs.length === 0 ? (
                <p className="py-6 text-center text-sm text-muted-foreground">Zatiaľ žiadne exporty.</p>
              ) : (
                    ...
                    <li key={j.id} className="flex items-center justify-between py-3">
                      <div className="min-w-0">
                        ...
                        <div className="mt-0.5 text-xs text-muted-foreground">
                          {j.invoice_count} faktúr · {new Date(j.created_at).toLocaleString("sk-SK")}
                          {j.date_from && j.date_to ? ` · ${j.date_from} → ${j.date_to}` : ""}
                        </div>
                      </div>
                      <button onClick={() => downloadJob(j)} className="inline-flex shrink-0 items-center gap-1 rounded-md border border-border px-3 py-1.5 text-sm hover:bg-secondary">
          ...
          <aside className="space-y-3">
            <div className="rounded-2xl border border-border bg-card p-5">
              ...
              <ul className="mt-3 space-y-2 text-sm">
                <li className="flex items-center justify-between rounded-lg border border-primary/40 bg-primary/5 px-3 py-2">
                  ...
                  <span className="rounded-full bg-primary/15 px-2 py-0.5 text-[10px] font-medium text-primary">Aktívne</span>
                </li>
                {["Omega", "Money S3", "Alfa Plus"].map((n) => (
                  <li key={n} className="flex items-center justify-between rounded-lg border border-dashed border-border px-3 py-2 text-muted-foreground">
                    <span>{n}</span>
            ...
            <div className="rounded-2xl border border-border bg-card p-5 text-sm text-muted-foreground">
              <p className="flex items-start gap-2"><ChevronRight className="mt-0.5 h-4 w-4 shrink-0 text-primary" />
                Vyberte obdobie a faktúry, ktoré chcete preniesť do účtovníctva. Súbor sa stiahne a uloží sa do histórie.
              </p>
            </div>
          </aside>
        </div>
