...
function ExportPage() {
  ...
  from0.setMonth(from0.getMonth() - 1);
  ...
  async function exportXlsx() {
    ...
    try {
      ...
      const cell = (v: any, type = "String") =>
        `<Cell><Data ss:Type="${type}">${xmlEsc(v)}</Data></Cell>`;
      ...
      const bodyRows = rows
        .map((r: any) => {
          ...
          return `<Row>${[
            cell(r.vehicles?.name ?? ""),
            cell(r.vehicles?.license_plate ?? ""),
            cell(r.trip_date),
            cell(r.start_location ?? ""),
            cell(r.end_location ?? ""),
        ...
        <div className="grid max-w-2xl gap-4">
          <label className="block">
            ...
            <select
              value={vehicle_id ?? ""}
              onChange={(e) =>
                navigate({ search: { vehicle_id: e.target.value || undefined } as any })
              }
          ...
          <div className="flex flex-wrap gap-2">
            ...
            <button
              disabled={busy}
              onClick={exportCsv}
              className="inline-flex items-center gap-2 rounded-md border border-border bg-card px-4 py-2 text-sm hover:bg-secondary disabled:opacity-60"
            >
          ...
          <p className="text-xs text-muted-foreground">
            PDF sa generuje cez tlač prehliadača. XLSX je vo formáte SpreadsheetML 2003 (.xls) —
            otvoria ho Excel aj LibreOffice.
          </p>
        </div>
      </PageBody>
    </>
